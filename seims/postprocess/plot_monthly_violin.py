...
def _monthly_stats(values):
    """对一个月的数值列表做稳健统计；空列表返回 None。"""
...
def print_fi_bi_by_month(month_to_fi, month_to_bi, decimals=4, print_raw=False):
    print("\n[Monthly FI/BI Summary]")
    ...
    for m in range(1, 13):
        fi_list = month_to_fi.get(m, [])
        bi_list = month_to_bi.get(m, [])

        fi_stat = _monthly_stats(fi_list)
        bi_stat = _monthly_stats(bi_list)

        # 收集全年数据（用于 overall mean）
        all_fi.extend([x for x in fi_list if x is not None and not np.isnan(x)])
        all_bi.extend([x for x in bi_list if x is not None and not np.isnan(x)])

        if fi_stat is None:
            fi_str = "FI: n=0"
        else:
            fi_month_mean[m] = fi_stat["mean"]
            fi_str = (
                f"FI: n={fi_stat['n']}, "
                f"mean={fi_stat['mean']:.{decimals}f}, "
                f"median={fi_stat['median']:.{decimals}f}, "
                f"std={fi_stat['std']:.{decimals}f}, "
                f"min={fi_stat['min']:.{decimals}f}, "
                f"p25={fi_stat['p25']:.{decimals}f}, "
                f"p75={fi_stat['p75']:.{decimals}f}, "
                f"max={fi_stat['max']:.{decimals}f}"
            )

        if bi_stat is None:
            bi_str = "BI: n=0"
        else:
            bi_month_mean[m] = bi_stat["mean"]
            bi_str = (
                f"BI: n={bi_stat['n']}, "
                f"mean={bi_stat['mean']:.{decimals}f}, "
                f"median={bi_stat['median']:.{decimals}f}, "
                f"std={bi_stat['std']:.{decimals}f}, "
                f"min={bi_stat['min']:.{decimals}f}, "
                f"p25={bi_stat['p25']:.{decimals}f}, "
                f"p75={bi_stat['p75']:.{decimals}f}, "
                f"max={bi_stat['max']:.{decimals}f}"
            )

        print(f"  Month {m:02d} | {fi_str} | {bi_str}")

        if print_raw:
            if len(fi_list) > 0:
                print(f"    FI raw: {[round(float(x), decimals) for x in fi_list]}")
            if len(bi_list) > 0:
                print(f"    BI raw: {[round(float(x), decimals) for x in bi_list]}")

    # ===== 新增：把“所有平均值”单独打印出来 =====
    print("\n[Monthly Means Only]")
    print("FI monthly mean:", {m: round(v, decimals) for m, v in fi_month_mean.items()})
    print("BI monthly mean:", {m: round(v, decimals) for m, v in bi_month_mean.items()})

    # 也给一个固定 12 个月顺序的列表（没数据的月份用 nan）
    fi_mean_list = [fi_month_mean.get(m, np.nan) for m in range(1, 13)]
    bi_mean_list = [bi_month_mean.get(m, np.nan) for m in range(1, 13)]
    print("FI mean list (Jan..Dec):", [None if np.isnan(x) else round(float(x), decimals) for x in fi_mean_list])
    print("BI mean list (Jan..Dec):", [None if np.isnan(x) else round(float(x), decimals) for x in bi_mean_list])

    # 全年总体平均值（把所有月份样本合并）
    overall_fi = float(np.mean(np.array(all_fi, dtype=float))) if len(all_fi) else np.nan
    overall_bi = float(np.mean(np.array(all_bi, dtype=float))) if len(all_bi) else np.nan
    print("\n[Overall Means Across All Days]")
    print(f"Overall FI mean = {overall_fi:.{decimals}f}  (n={len(all_fi)})")
    print(f"Overall BI mean = {overall_bi:.{decimals}f}  (n={len(all_bi)})")
...
def run_monthly_fi_bi_half_violin(
    ...
    # ✅ 新增：按月统计打印
    print_fi_bi_by_month(month_to_fi, month_to_bi, decimals=4, print_raw=False)
    ### 半小提琴图
...
# =========================
# 1) 计算观测淹没面积（从 obs tif）
# =========================
WGS84_R = 6378137.0  # meter

def calc_area_mask_wgs84_geographic(
    tif_path,
    mask,                 # bool array: True=计入面积
    unit="m2",            # "m2" or "km2"
):
    """
    在 WGS84 经纬度栅格上，给定一个布尔 mask，计算其真实面积。
    要求栅格是规则经纬网（常见的经纬度 GeoTIFF）。
    """
    with rasterio.open(tif_path) as src:
        ...
        # 像元角点坐标（度）
        # transform.a = pixel width in degrees (lon)
        # transform.e = pixel height in degrees (lat, usually negative)
        ...
        rows = np.arange(h)
        # 取每行第0列像元中心的纬度
        _, lat_center = xy(transform, rows, np.zeros_like(rows), offset="center")
        lat_center = np.array(lat_center, dtype=float)

        # 对每一行计算“该行每个像元”的面积（m²）
        # φ1 = center - dlat/2, φ2 = center + dlat/2
        phi1 = np.deg2rad(lat_center - dlat / 2.0)
        phi2 = np.deg2rad(lat_center + dlat / 2.0)
        dlam = np.deg2rad(dlon)

        # 行像元面积（m²），每行一个值
        area_per_pixel_row = (WGS84_R ** 2) * dlam * (np.sin(phi2) - np.sin(phi1))
        area_per_pixel_row = np.abs(area_per_pixel_row)  # 保证正数

        # 把行面积扩展到二维（h,w），然后对 mask 求和
        area2d = area_per_pixel_row[:, None]  # (h,1) broadcast to (h,w)
        total_m2 = float(np.sum(area2d * mask.astype(np.float32)))

        if unit == "km2":
            return total_m2 / 1e6
        return total_m2
def calc_inundation_area_from_tif(tif_path, thresh=0.0, unit="km2"):
    with rasterio.open(tif_path) as src:
        arr = src.read(1)
        nodata = src.nodata
        if nodata is not None:
            mask = (arr != nodata) & (arr > thresh)
        else:
            mask = (arr > thresh)
    return calc_area_mask_wgs84_geographic(tif_path, mask, unit=unit)


# =========================
# 2) 收集逐日记录：date, FI, BI, obs_area
# =========================
@dataclass
class DailyFiBiAreaRecord:
    d: date
    fi: float
    bi: float
    obs_area: float
...
def collect_fi_bi_area_by_date(
    sim_dir: Union[str, Path],
    obs_dir: Union[str, Path],
    sim_thresh: float = 0.0,
    obs_thresh: float = 0.0,
    debug: bool = True
) -> List[DailyFiBiAreaRecord]:
    """
    与你现有的 collect_fi_bi_by_month 思路一致：
    - 用文件名解析日期
    - 按日期配对 sim/obs
    - 计算 FI/BI（复用你已有的计算函数）
    - 同时计算 obs 的淹没面积 obs_area

    你需要提供/已有：
      - parse_date_from_name(filename) -> date
      - calc_fi_bi(sim_tif_path, obs_tif_path, sim_thresh, obs_thresh) -> (fi, bi)
        （名字不同就替换掉）
    """
    sim_dir = Path(sim_dir)
    obs_dir = Path(obs_dir)
    ...
    for p in obs_dir.glob("*.tif"):
        try:
            d = parse_date_from_name(p.name)  # <-- 你已有
            obs_map[d] = p
        except Exception:
            if debug:
                print(f"[obs] 跳过无法解析日期的文件: {p.name}")

    records: List[DailyFiBiAreaRecord] = []
    miss = 0

    for sim_p in sim_dir.glob("*.tif"):
        try:
            d = parse_date_from_name(sim_p.name)  # <-- 你已有
        except Exception:
            if debug:
                print(f"[sim] 跳过无法解析日期的文件: {sim_p.name}")
            continue
        ...
        if obs_p is None:
            miss += 1
            if debug:
                print(f"[pair] 缺少观测文件，日期 {d}，sim={sim_p.name}")
            continue

        # === 计算 FI/BI：这里复用你已有的 FI/BI 计算逻辑 ===
        sim_arr, obs_arr, sim_nodata, obs_nodata = read_and_align_rasters(sim_p, obs_p, debug=False)
        fi, bi = compute_fi_bi(
            sim_arr, obs_arr,
            sim_nodata=sim_nodata,
        ...
        # === 计算 obs 淹没面积 ===
        obs_area = calc_inundation_area_from_tif(obs_p, thresh=obs_thresh)

        records.append(DailyFiBiAreaRecord(d=d, fi=float(fi), bi=float(bi), obs_area=float(obs_area)))

    # 按日期排序
    records.sort(key=lambda r: r.d)

    if debug:
        print(f"\n[collect] 逐日记录数={len(records)}, 未配对数={miss}")
        if records:
            print(f"[collect] 日期范围：{records[0].d} ~ {records[-1].d}")

    return records


# =========================
# 3) 绘图：obs_area vs FI / obs_area vs BI（两栏子图）
# =========================
def plot_obs_area_vs_fi_bi_two_panels(
    records: List[DailyFiBiAreaRecord],
    out_png: Union[str, Path],
    out_pdf: Optional[Union[str, Path]] = None,
    title: str = "FI/BI vs observed inundation area",
    area_unit: str = "km²",
    area_scale: float = 1.0,  # 例如 km²：1e-6
    point_size: float = 10,
    point_alpha: float = 0.6,
    show_grid: bool = True,
    xlim_area: Optional[Tuple[float, float]] = None,
    ylim_fi: Optional[Tuple[float, float]] = None,
    ylim_bi: Optional[Tuple[float, float]] = None,
    # ✅ 字体控制
    ...
    """
    if not records:
        raise ValueError("records 为空，无法绘图。")

    plt.rcParams.update({
        "font.family": "serif",
        "font.serif": ["Times New Roman", "Times", "DejaVu Serif"],
        "font.size": base_fontsize,
        "pdf.fonttype": 42,  # 让 PDF 字体更“可编辑/可嵌入”
        "ps.fonttype": 42,
    })

    fi = np.array([r.fi for r in records], dtype=float)
    bi = np.array([r.bi for r in records], dtype=float)
    # ✅ BI 分布统计（忽略 NaN）
    bi_valid = bi[np.isfinite(bi)]
    n = bi_valid.size
    p01 = float(np.mean(np.abs(bi_valid) <= 0.1) * 100.0) if n > 0 else 0.0
    p02 = float(np.mean(np.abs(bi_valid) <= 0.2) * 100.0) if n > 0 else 0.0

    print("[BI stats] n = {}".format(n))
    print("[BI stats] |BI| <= 0.1 : {:.2f}%".format(p01))
    print("[BI stats] |BI| <= 0.2 : {:.2f}%".format(p02))
    area = np.array([r.obs_area for r in records], dtype=float) * area_scale

    # ✅ FI 统计：当观测淹没面积 > 2500 km² 时，FI > 0.6 的比例
    # 注意：这里默认 area 的单位就是 km²（与你的 area_unit 对齐）
    mask_large = np.isfinite(area) & (area > 2500.0) & np.isfinite(fi)
    n_large = int(np.count_nonzero(mask_large))

    if n_large > 0:
        pct_fi_gt_06 = float(np.mean(fi[mask_large] > 0.6) * 100.0)
    else:
        pct_fi_gt_06 = float("nan")

    print("[FI stats] Obs area > 2500 km²: n = {}".format(n_large))
    print("[FI stats] Among them, FI > 0.6: {:.2f}%".format(pct_fi_gt_06))

    fig, axes = plt.subplots(1, 2, figsize=(12.5, 4.8), dpi=1000)

    # --- panel 1: Area vs FI ---
    ax = axes[0]
    ax.scatter(area, fi, s=point_size, alpha=point_alpha)
    ax.set_xlabel("Observed inundation area ({})".format(area_unit), fontsize=label_fontsize)
    ax.set_ylabel("FI", fontsize=label_fontsize)
    # ax.set_title("FI vs obs area", fontsize=title_fontsize)
    ax.tick_params(axis="both", labelsize=tick_fontsize)

    if show_grid:
        ax.grid(True, linestyle="--", linewidth=0.6, alpha=0.4)
    if xlim_area is not None:
        ax.set_xlim(*xlim_area)
    if ylim_fi is not None:
        ax.set_ylim(*ylim_fi)
    # 去掉上/右边框
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    # 可选：把次网格也画出来（更像你现在的风格）
    ax.grid(True, which="minor", axis="y", linestyle="--", linewidth=0.6, alpha=0.25)

    # --- panel 2: Area vs BI ---
    ax = axes[1]
    ax.scatter(area, bi, s=point_size, alpha=point_alpha)
    ax.set_xlabel("Observed inundation area ({})".format(area_unit), fontsize=label_fontsize)
    ax.set_ylabel("BI", fontsize=label_fontsize)
    # ax.set_title("BI vs obs area", fontsize=title_fontsize)
    ax.tick_params(axis="both", labelsize=tick_fontsize)
    ...
    # 可选：把次网格也画出来（更像你现在的风格）
    ax.grid(True, which="minor", axis="y", linestyle="--", linewidth=0.6, alpha=0.25)
    # fig.suptitle(title, y=1.02, fontsize=suptitle_fontsize)
    fig.tight_layout()
...
# =========================
# 4) 入口函数：仿照你 run_monthly_* 的写法
# =========================
def run_obs_area_vs_fi_bi_scatter(
    sim_dir: Union[str, Path],
    obs_dir: Union[str, Path],
    out_png: Union[str, Path],
    out_pdf: Optional[Union[str, Path]] = None,
    ...
    records = collect_fi_bi_area_by_date(
        sim_dir=sim_dir,
        obs_dir=obs_dir,
        sim_thresh=sim_thresh,
        obs_thresh=obs_thresh,
    ...
    if debug and records:
        print("\n[preview] 前5条记录：")
        for r in records[:5]:
            print(f"  {r.d}: FI={r.fi:.4f}, BI={r.bi:.4f}, obs_area={r.obs_area*area_scale:.4f} {area_unit}")

    plot_obs_area_vs_fi_bi_two_panels(
        records=records,
        out_png=out_png,
        out_pdf=out_pdf,
        title="FI/BI vs observed inundation area",
        area_unit=area_unit,
        ...
        # ✅ 新参数名
        xlim_area=None,  # 或者 (0, 3000) 之类，取决于你的面积单位/范围
        ylim_fi=(0.0, 1.0),  # 你希望 FI 的 y 轴范围
        ylim_bi=None,  # 例如 (-0.5, 0.5)，不限制就 None
...
if __name__ == '__main__':
    if os.name == 'nt':  # Windows
        ...
        shp_dir = r"G:\program\seims\SEIMS_HAND\data\poyang_lake1\sci_figure_study_region\ubbasin_1171_upstream_only_inundation_disolved3.shp"
