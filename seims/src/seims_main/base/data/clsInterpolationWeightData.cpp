...
ItpWeightData::ItpWeightData(MongoGridFs* gfs, const string& filename) :
    ...
    n_rows_(-1), n_cols_(-1), n_cols_weight_(-1), initialized_(false) {// by wanghaocheng , n_cols_weight_(-1)
...
//by wanghaocheng
void ItpWeightData::GetWeightIdData2D(int* n, int* n_stations, float*** data) {
    *n = n_rows_;
    //*n_stations = n_cols_;
    *n_stations = n_cols_weight_;
    if (nullptr == itp_weight_data2d_) {
        Initialize2DArray(n_rows_, n_cols_weight_, itp_weight_data2d_, 0.);
    }
    int index = 0;
    for (int i = 0; i < n_rows_; i++) {
        for (int j = 0; j < n_cols_weight_; j++) {
            index = i * (n_cols_weight_)+j;
            itp_weight_data2d_[i][j] = itp_weight_data_[index];
        }
    }
...
bool ItpWeightData::ReadFromMongoDB(MongoGridFs* gfs, const string& filename) {
    ...
    GetNumericFromBson(md, MONG_GRIDFS_NUM_SITE_WEIGHT, n_cols_weight_);// by wanghaocheng
    ...
    Initialize1DArray(n_rows_ * n_cols_weight_, itp_weight_data_, tmp_float_weight);
