...
const int REACH_PARAM_NUM = 78; /// Numerical parameters, except GROUP related
const char* REACH_PARAM_NAME[] = {
    ...
    REACH_SURFRDOC,REACH_LATRDOC,REACH_GWDRDOC
};
    ...
    for (int i = 0; i < REACH_PARAM_NUM; i++) {
        ...
        if (it != caliparams_map.end()) {
            ...
            for (auto it2 = reaches_obj_.begin(); it2 != reaches_obj_.end(); ++it2) {
                ...
                it2->second->Set(REACH_PARAM_NAME[i], it->second->GetAdjustedValueWithFID(it2->first,pre_value));
            }
        }
    }
