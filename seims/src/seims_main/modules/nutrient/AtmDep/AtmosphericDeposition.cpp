...
AtmosphericDeposition::AtmosphericDeposition() :
    ...
    m_soilNH4(nullptr), m_soilNO3(nullptr),m_soilDOC(nullptr) {
}
...
void AtmosphericDeposition::Set2DData(const char* key, const int nrows, const int ncols, float** data) {
    ...
    else if (StringMatch(sk, VAR_SOL_BMC)) m_soilDOC = data;
    else {
...
#pragma omp parallel for
    for (int i = 0; i < m_nCells; i++) {
        ...
        float m_rainDOCConc=1.2f;
        float addrdoc = 0.01f * m_rainDOCConc * m_pcp[i] /10.f; //to g/m2
        m_soilDOC[i][0] += addrdoc;
    }
    return 0;
}
