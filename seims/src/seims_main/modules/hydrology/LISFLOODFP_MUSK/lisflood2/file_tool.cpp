...
#include <gdal.h>
#include <gdal_priv.h>
// xiaodw, for distributed rainfall ,reading count of tif file in a folder
#ifdef WIN32
#include <windows.h>
#else
#include <dirent.h>
...
///
/// if data is NULL, data is allocated within this function and must be freed by user with memory_free_legacy
///
void read_file_asc(FILE *fp, const NUMERIC_TYPE nodata, int * num_cols, int * num_rows, NUMERIC_TYPE** data, NUMERIC_TYPE * xllcorner, NUMERIC_TYPE * yllcorner, NUMERIC_TYPE * cell_size)
{
	char dum[800];
	NUMERIC_TYPE no_data_value = NULLVAL;
	int i, j;
	...
	fscanf(fp, "%s %i", dum, &ncols);
	fscanf(fp, "%s %i", dum, &nrows);
	fscanf(fp, "%s %" NUM_FMT"", dum, xllcorner);
	fscanf(fp, "%s %" NUM_FMT"", dum, yllcorner);
	fscanf(fp, "%s %" NUM_FMT"", dum, cell_size);
	...
	*num_rows = nrows;
	...
	if (*data == NULL)
	{
		*data = memory_allocate_numeric_legacy(ncols * nrows);
	}

	NUMERIC_TYPE *data_grid = *data;
	...
	for (j = 0; j < nrows; j++)
	{
		for (i = 0; i < ncols; i++)
		{
			int index = i + j*ncols;
			NUMERIC_TYPE value;
			fscanf(fp, "%" NUM_FMT"", &value);
			if ((int)value == (int)no_data_value)
				value = nodata;// 1e10;
			data_grid[index] = value;
		}
	}
}
...
void read_file_asc(const char * filename, const NUMERIC_TYPE nodata, int * num_cols, int * num_rows, NUMERIC_TYPE** data, NUMERIC_TYPE * xllcorner, NUMERIC_TYPE * yllcorner, NUMERIC_TYPE * cell_size)
{
	...
	read_file_asc(fp, nodata, num_cols, num_rows, data, xllcorner, yllcorner, cell_size);
}

void read_file_bin(FILE *fp, const char * filename, const NUMERIC_TYPE nodata, int * num_cols, int * num_rows, NUMERIC_TYPE** data, NUMERIC_TYPE * xllcorner, NUMERIC_TYPE * yllcorner, NUMERIC_TYPE * cell_size)
{
	NUMERIC_TYPE no_data_value = NULLVAL;
	int i, j;
	int ncols, nrows;
	...
	int count;
	int header_int[2];
	NUMERIC_TYPE header_numeric[4];

	count = fread(header_int, sizeof(int), 2, fp);
	if (count != 2)
	{
		printf("File read error %s\n", filename);
		exit(0);
	}
	count = fread(header_numeric, sizeof(NUMERIC_TYPE), 4, fp);
	if (count != 4)
	{
		printf("File read error %s\n", filename);
		exit(0);
	...
	if (file_length != 8 + (4 + ncols * nrows) * sizeof(NUMERIC_TYPE))
	{
		printf("File read error: length mismatch, possible incorrect precision. Expect: %s %s\n", NUMERIC_TYPE_NAME, filename);
		exit(0);
	}
	...
	*xllcorner = header_numeric[0];
	*yllcorner = header_numeric[1];
	*cell_size = header_numeric[2];
	no_data_value = header_numeric[3];
	...
	*num_rows = nrows;
	...
	count = fread(*data, sizeof(NUMERIC_TYPE), ncols * nrows, fp);
	...
	if (no_data_value != nodata)
	{
		for (j = 0; j < nrows; j++)
		{
			...
			{
				int index = i + j * ncols;
				if (data_grid[index] == no_data_value)
					data_grid[index] = nodata;// 1e10;
			}
		}
	}
}
...
void read_file_bin(const char * filename, const NUMERIC_TYPE nodata, int * num_cols, int * num_rows, NUMERIC_TYPE** data, NUMERIC_TYPE * xllcorner, NUMERIC_TYPE * yllcorner, NUMERIC_TYPE * cell_size)
{
	...
	read_file_bin(fp, filename, nodata, num_cols, num_rows, data, xllcorner, yllcorner, cell_size);
}
...
{
	// check if there is a savenumber to add and create tar_file_name
...
void make_tif_filename(const char *root, const int SaveNumber, const char *extension, const OutputParams* output_params, char * filename,char  *file_type)
{
	// check if there is a savenumber to add and create tar_file_name
	if (SaveNumber >= 0 && SaveNumber <= 9999)
		sprintf(filename, "%s-%.4d-%s%s", root, SaveNumber, file_type, extension);
	else if (SaveNumber > 9999)
		sprintf(filename, "%s-%d-%s%s", root, SaveNumber,file_type, extension);
	else
		sprintf(filename, "%s%s-%s", root, file_type, extension);
...
//void writeJPG(const char *root, const int SaveNumber,
//	int netcdf_varid, const char *extension,
//	NUMERIC_TYPE *data,
//	const int grid_cols, const int grid_rows, const NUMERIC_TYPE xllcorner, const NUMERIC_TYPE yllcorner,
//	const NUMERIC_TYPE cell_size, OutputParams* output_params,  char  *file_type) {
//
//	char tar_file_name[1024];
//	make_tif_filename(root, SaveNumber, extension, output_params, tar_file_name, file_type);
//	Mat img(grid_rows, grid_cols, CV_8UC1);
//	for (int i = 0; i < grid_rows; i++)
//		for (int j = 0; j < grid_cols; j++)
//			img.at<uchar>(i, j) = data[i*grid_cols+j];
//
//	imwrite(tar_file_name, img);
//}
...
void writeTiF(const char *root, const int SaveNumber,
	int netcdf_varid, const char *extension,
	NUMERIC_TYPE *data,
	const int grid_cols, const int grid_rows, const NUMERIC_TYPE xllcorner, const NUMERIC_TYPE yllcorner, 
	const NUMERIC_TYPE cell_size, OutputParams* output_params, const char* src_file_name, char  *file_type) {
	char tar_file_name[1024];
	make_tif_filename(root, SaveNumber, extension, output_params, tar_file_name,file_type);
	GDALDataType dateType = GDT_Float32;
	writeTIFByGdal(src_file_name,tar_file_name, grid_cols, grid_rows,1, dateType,data);
}

bool writeTIFByGdal(const char* src_file_name,const char* tar_file_name, int width, int height, int bands, GDALDataType dataType, NUMERIC_TYPE * data)
{
	...
	if (poDriver == NULL) {
		cout << "Error: cannot get driver." << endl;
		return 0;
	}
	//char** papszMetadata = NULL;
	//GDALDataset* poDstDS = poDriver->Create(tar_file_name, width, height, bands, dataType, papszMetadata);
	//if (poDstDS == NULL) {
	//	cout << "Error: cannot create dataset." << endl;
	//	return 0;
	//}
	//for (int i = 0; i < bands; i++) {
	//	poDstDS->GetRasterBand(i + 1)->RasterIO(GF_Write, 0, 0, width, height,
	//		data, width, height, dataType,
	//		0, 0);
	//}
	//GDALClose((GDALDatasetH)poDstDS);
	GDALDataset* poSrcDS = (GDALDataset*)GDALOpen(src_file_name, GA_ReadOnly);
	GDALDataset* poDstDS;
	...
	string out_file = tar_file_name;
	// 这里有个bug：复制栅格元数据的时候会连最大最小值一起复制过来，导入到arcgis里显示会有问题
	...
	poOutBand->RasterIO(GF_Write, 0, 0, width, height, data, width, height, GDT_Float64, 0, 0);
	//poOutBand->SetNoDataValue(TIF_NO_DATA);
...
//bool readTIFNoGdal(const char* filename, NUMERIC_TYPE * data) {
...
//	{
//		cout << "Big endian byte order is not supported." << endl;
//		return 0;
//	}
//
//	// Check the version.
//	if (version != 42)
//	{
//		cout << "Invalid TIFF version." << endl;
//		return 0;
//	}
//
//	// Read the IFD.
...
//
//		file.read((char *)&tag_id, sizeof(tag_id));
//		file.read((char *)&data_type, sizeof(data_type));
//		file.read((char *)&num_values, sizeof(num_values));
//		file.read((char *)&value_or_offset, sizeof(value_or_offset));
//
//		switch (tag_id)
//		{
//		case 256:
//			// ImageWidth
//			//nXSize = value_or_offset;
//			cout << value_or_offset;
//			break;
//
//		case 257:
//			// ImageLength
...
//			// StripOffsets
//			if (num_values == 1)
//				strip_offsets.push_back(value_or_offset);
//			else
//			{
//				uint32_t *offsets = new uint32_t[num_values];
//				file.seekg(value_or_offset);
//				file.read((char *)offsets, num_values * sizeof(uint32_t));
//				for (int j = 0; j < num_values; j++)
//					strip_offsets.push_back(offsets[j]);
//				delete[] offsets;
//			}
//			break;
//
...
//			// StripByteCounts
//			if (num_values == 1)
//				strip_byte_counts.push_back(value_or_offset);
//			else
//			{
//				uint32_t *byte_counts = new uint32_t[num_values];
//				file.seekg(value_or_offset);
//				file.read((char *)byte_counts, num_values * sizeof(uint32_t));
//				for (int j = 0; j < num_values; j++)
//					strip_byte_counts.push_back(byte_counts[j]);
//				delete[] byte_counts;
//			}
//			break;
//
...
//	}
//
//	int nXSize = strip_byte_counts[0] / strip_offsets.size();
//	int nYSize = strip_offsets.size();
//
//	double *pafScanline = new double[nXSize * nYSize];
//	for (int i = 0; i < strip_offsets.size(); i++)
//	{
//		file.seekg(strip_offsets[i]);
//		char *buf = new char[strip_byte_counts[i]];
//		file.read(buf, strip_byte_counts[i]);
//
//		for (int j = 0; j < nXSize; j++)
//			pafScanline[i * nXSize + j] = ((double *)buf)[j];
//
//		delete[] buf;
//	}
...
bool readTIFByGdal(const char* file_path_name, NUMERIC_TYPE* data) {
	// 设置GDAL配置，支持中文路径
	...
	// 打开GDAL数据集
	GDALDataset* poDataset = (GDALDataset*)GDALOpen(file_path_name, GA_ReadOnly);
	...
	//std::cout << nImgSizeX << " * " << nImgSizeY << std::endl;

	// 获取坐标变换系数
	double trans[6];
	CPLErr err = poDataset->GetGeoTransform(trans);
	if (err == CE_None) {
		//std::cout << "Coordinate transform coefficients: ";
		//for (const auto& t : trans)
		//	std::cout << t << " ";
		//std::cout << std::endl;
	}
	else {
		std::cout << "Failed to get coordinate transform coefficients." << std::endl;
	}
	...
	for (int bandIndex = 1; bandIndex <= bandCount; bandIndex++) {
		...
		// 读取图像数据
		std::vector<float> scanline(nImgSizeX * nImgSizeY);
		CPLErr error = poBand->RasterIO(GF_Read, 0, 0, nImgSizeX, nImgSizeY, scanline.data(), nImgSizeX, nImgSizeY, GDT_Float32, 0, 0);
		if (error == CE_Failure) {
		...
		for (int i = 0; i < nImgSizeY; i++) {
			for (int j = 0; j < nImgSizeX; j++) {
				num_image_size++;
				data[i * nImgSizeX + j] = scanline[i * nImgSizeX + j];
				if (scanline[i * nImgSizeX + j] > 0) {
					valid_cell_num++;
					//cout << i << "   " << j << "   " << data[i * nImgSizeX + j] << endl;
				}
			}
		}
		//std::cout << "Total: " << num_image_size << " Valid: " << valid_cell_num << std::endl;
	}
...
vector<string> getTimesFromFiles(const string& folderPath, const string& fileExtension) {
	vector<string> times;
	...
	string searchPath = folderPath + "\\*" + fileExtension;
	...
	do {
		...
		if (filename.length() >= 13 + 10) {
			string timePart = filename.substr(13, 10);
			times.push_back(timePart);
		}
	...
	if ((dir = opendir(folderPath.c_str())) != NULL) {
		while ((ent = readdir(dir)) != NULL) {
			...
			if (stat(fullFilePath.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
				if (filename.length() >= fileExtension.length() &&
					...
					if (filename.length() >= 12 + 10) {
						string timePart = filename.substr(12, 10);
						times.push_back(timePart);
					}
				}
			}
...
char* createTifFilename(int timestamp) {
	// 将timestep转换为time_t类型
	time_t rawtime = static_cast<time_t>(timestamp);

	// 将time_t转换为tm结构体
	tm* timeinfo = gmtime(&rawtime);

	// 将tm结构体转换为字符串格式YYYYMMDDHH
	ostringstream oss;
	oss << RAIN_FILE_PREFIX
		<< setw(4) << setfill('0') << (timeinfo->tm_year + 1900)  // 年
		<< setw(2) << setfill('0') << (timeinfo->tm_mon + 1)      // 月
		<< setw(2) << setfill('0') << timeinfo->tm_mday           // 日
		<< setw(2) << setfill('0') << timeinfo->tm_hour           // 时
		<< setw(2) << setfill('0') << timeinfo->tm_min            // 分
		<< TIF;
...
void buildFullFilePath(char* dest, const char* folderPath, const char* fileName) {
	// 检查folderPath结尾是否有路径分隔符
	size_t folderPathLen = strlen(folderPath);
	bool hasTrailingSlash = (folderPathLen > 0 && (folderPath[folderPathLen - 1] == '/' || folderPath[folderPathLen - 1] == '\\'));

	if (hasTrailingSlash) {
		// 如果有路径分隔符，直接拼接
		snprintf(dest, 256, "%s%s", folderPath, fileName);
	}
	else {
		// 如果没有路径分隔符，添加路径分隔符后再拼接
#ifdef _WIN32
		snprintf(dest, 256, "%s\\%s", folderPath, fileName); // Windows 使用反斜杠
#else
		std::snprintf(dest, 256, "%s/%s", folderPath, fileName);  // POSIX 使用斜杠
#endif
	}
}
