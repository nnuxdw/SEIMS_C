...
bool writeTIFByGdal(const char* src_file_name, const char* file_path_name, int width, int height, int bands, GDALDataType dataType, NUMERIC_TYPE * data);
void writeTiF(const char *root, const int SaveNumber,
	int netcdf_varid, const char *extension,
	NUMERIC_TYPE *data,
	const int grid_cols, const int grid_rows, const NUMERIC_TYPE xllcorner, const NUMERIC_TYPE yllcorner, const NUMERIC_TYPE cell_size, OutputParams* output_params, const char* src_file_name, char  *file_type);
