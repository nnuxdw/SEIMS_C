...
void write_regular_output(const char * resrootname, const int grid_cols, const int grid_rows, const int grid_cols_padded,
	...
	Solver *Solverptr, States *Statesptr, Pars *Parptr, Fnames *Fnameptr,BoundaryCondition *boundary_cond, SGCprams *SGCptr,
	...
	const int save_glacier_thickness,
	const int save_snow_thickness,
	const int save_water_table)
{
	...
	else
	{
		...
		// xdw add, support snow and glacier thickness tif output
		GDALDataType dateType = GDT_Float32;
		//char  src_file_name[] = "G:\\program\\longxiazailongba\\data\\simulate\\20230710\\prepare\\dembnk.tif";
		if (save_water_table == ON)
		{
			for (int j = 0; j < grid_rows; j++)
			{
				for (int i = 0; i < grid_cols; i++)
				{
					...
					NUMERIC_TYPE temp = Parptr->tableDepthPD[index_padded];
			...
			writeTiF(resrootname, save_number,
				output_params->netcdf_state.varid_depth, ".tif",
				tmp_grid1, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, Fnameptr->tif_src_file, "water_table");
			//write_grid(resrootname, save_number,
			//	output_params->netcdf_state.varid_depth, ".water_table",
			//	Parptr->tableDepthPD, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params);
		...
		if (save_water_table == ON)
		{
			for (int j = 0; j < grid_rows; j++)
			{
				for (int i = 0; i < grid_cols; i++)
				{
					...
					int index = i + j * grid_cols;
					// calculate depth on floodplain
					NUMERIC_TYPE temp = Parptr->satFlow2SurfPD[index_padded];
					// if water level is below floodplain set depth to zero
					//if (temp <= depth_thresh)
					//	temp = C(0.0);
					tmp_grid1[index] = temp;
			...
			writeTiF(resrootname, save_number,
				output_params->netcdf_state.varid_depth, ".tif",
				tmp_grid1, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, Fnameptr->tif_src_file, "sat2surf");
			//write_grid(resrootname, save_number,
			//	output_params->netcdf_state.varid_depth, ".water_table",
			//	Parptr->tableDepthPD, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params);
		...
		if (save_glacier_thickness == ON)
		{
			if (Statesptr->save_in_tif == ON)
			{
				writeTiF(resrootname, save_number,
					output_params->netcdf_state.varid_depth, ".tif",
					Parptr->glacier, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, Fnameptr->tif_src_file,"glacier");
			}
			
			write_grid(resrootname, save_number,
				output_params->netcdf_state.varid_depth, ".glacier",
				Parptr->glacier, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params);
			

		}
		if (save_snow_thickness == ON)
		{
			...
			if (Statesptr->save_in_tif == ON)
			{
				writeTiF(resrootname, save_number,
					output_params->netcdf_state.varid_depth, ".tif",
					Parptr->snow, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, Fnameptr->tif_src_file,"snow");
			}
			
			write_grid(resrootname, save_number,
				output_params->netcdf_state.varid_depth, ".snow",
				Parptr->snow, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params);
		...
		if (save_depth == ON)
		{
			...
			for (int j = 0; j < grid_rows; j++)
			{
				for (int i = 0; i < grid_cols; i++)
				{
					int index_padded = i + j * grid_cols_padded;
					int index = i + j * grid_cols;
					if (dem_grid[index_padded] != DEM_NO_DATA)
					{
			...
			if (Statesptr->save_in_tif == ON)
			{
				writeTiF(resrootname, save_number,
					output_params->netcdf_state.varid_depth, ".tif",
					tmp_grid1, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, Fnameptr->tif_src_file,"wd");
			}
			//if (Statesptr->save_in_jpg == ON)
			//{
			//	writeJPG(resrootname, save_number,
			//		output_params->netcdf_state.varid_depth, ".jpg",
			//		tmp_grid1, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, "wd");
			...
			// test poi 
			//tmp_grid1[455 + 148 * grid_cols] = 9998.0;
			//tmp_grid1[455 + 148 * grid_cols] = 9998.0;
			//tmp_grid1[432 + 152 * grid_cols] = 9999.0;
			//tmp_grid1[576 + 88* grid_cols] = 10000.0;
			//if (Statesptr->save_in_tif == ON)
			//{
			//	
			//	writeTIFByGdal();
		...
		if (save_elev == ON)
		{
			for (int j = 0; j < grid_rows; j++)
			{
				for (int i = 0; i < grid_cols; i++)
				{
					...
					if (dem_min > dem_grid[index_padded])
					{
						dem_min = dem_grid[index_padded];
					}
					tmp_grid1[index] = temp;
				}
			}
			...
			if (Statesptr->save_in_tif == ON)
			{
				writeTiF(resrootname, save_number,
					output_params->netcdf_state.varid_depth, ".tif",
					tmp_grid1, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, Fnameptr->tif_src_file, "elev");
			}
			else
			{
				write_grid(resrootname, save_number,
			...
			//NUMERIC_TYPE* elev_min = new NUMERIC_TYPE[grid_rows* grid_cols];
			//if (save_elev == ON) {
			//	// 临时给进帅写的，输出每个栅格水位-dem最小高程
			//	for (int j = 0; j < grid_rows; j++)
			//	{
			//		for (int i = 0; i < grid_cols; i++)
			//		{
			...
			//			NUMERIC_TYPE temp = h_grid[index_padded];
			//			// if water level is below floodplain or sub-grid-channel set depth to NULLVAL
			//			elev_min[index] = tmp_grid1[index] - dem_min;
			//		}
			//	}
			//	if (Statesptr->save_in_tif == ON)
			//	{
			//		writeTiF(resrootname, save_number,
			//			output_params->netcdf_state.varid_depth, ".tif",
			//			elev_min, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params, src_file_name, "elev-min");
			//	}
...
#pragma ivdep
				for (int cell_i = 0; cell_i < sub_grid_layout->cell_row_count[j]; cell_i++)
				{
					...
					if (sub_grid_layout->cell_info.sg_cell_SGC_is_large[cell_index])
					{
						...
						// xdw modify, 当次网格宽度大于0.5*(dx+dy)时，也强制输出坡面流速
...
#pragma ivdep
				for (int cell_i = 0; cell_i < sub_grid_layout->cell_row_count[j]; cell_i++)
				{
					...
					if (sub_grid_layout->cell_info.sg_cell_SGC_is_large[cell_index])
					{
						...
						// todo3: 这里当初是否想错了？为什么程序原本不输出vx和vy？
						// xdw modify, 当次网格宽度大于0.5*(dx+dy)时，也强制输出坡面流速
						//tmp_grid1[grid_index] = C(0.0);
					}
				}
		...
		if (save_sub_grid == ON && Statesptr->SGCvoutput == ON)
		{
			...
			// xdw modify, 这里计算SGCVy，应该对应tmp_grid2
			//get_sub_grid_values(grid_cols, grid_rows, qy_grid_cols, tmp_grid1, sub_grid_state->sg_velocity, sub_grid_layout, false);
			get_sub_grid_values(grid_cols, grid_rows, qy_grid_cols, tmp_grid2, sub_grid_state->sg_velocity, sub_grid_layout, false);
...
#pragma ivdep
				...
				{
					int cell_index = cell_i + sg_list_row_start;
					// 如果subgrid河道宽度>0.5(x+y)，则输出SGCVc
					if (sub_grid_layout->cell_info.sg_cell_SGC_is_large[cell_index])
					{
