...
void CheckParam(char* param_name, char* param_value_ptr, int line_number, Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Solver *Solverptr, int verbose, int mode)
{
	...
	if (read_numeric_param(param_name, line_number, param_value_ptr, "saveint", &Parptr->SaveInt, verbose, mode))
		return; 
	if (read_numeric_param(param_name, line_number, param_value_ptr, "poisaveint", &Parptr->PoiSaveInt, verbose, mode))
		return;
	// save wd, elev, wdfp files in tif format
	if (read_empty_param(param_name, line_number, param_value_ptr, "save_in_tif", verbose, mode))
	{
		Statesptr->save_in_tif = ON;

		return;
	}
	if (read_string_param(param_name, line_number, param_value_ptr, "tif_src_file", Fnameptr->tif_src_file, verbose, mode))
	{
		printf("tif_src_file: %s\n", Fnameptr->tif_src_file);
		return;
	}

	if (read_empty_param(param_name, line_number, param_value_ptr, "save_in_jpg", verbose, mode))
	{
		Statesptr->save_in_jpg = ON;
		return;
	}
	...
	if (read_empty_param(param_name, line_number, param_value_ptr, "save_snow_thickness", verbose, mode))
	{
		Statesptr->save_snow_thickness = ON;
		return;
	}
	if (read_empty_param(param_name, line_number, param_value_ptr, "save_glacier_thickness", verbose, mode))
	{
		Statesptr->save_glacier_thickness = ON;
		return;
	}
	if (read_empty_param(param_name, line_number, param_value_ptr, "save_table_depth", verbose, mode))
	{
		Statesptr->save_table_depth = ON;
		return;
	}
	...
	// xdw modify, support green-ampt mode, specify a param value or a distribution file related to green-ampt(eg. soil propertites)
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_green_ampt_singlelayer", verbose, mode)) // Enable green_ampt
	{
		Statesptr->use_green_ampt_singlelayer = ON;
		return;
	}
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_green_ampt_multilayer", verbose, mode)) // Enable green_ampt
	{
		Statesptr->use_green_ampt_multilayer = ON;
		return;
	}
	// ks
	...
	{
		Statesptr->use_interflow_singlelayer = ON;
		return;
	}
	if (read_string_param(param_name, line_number, param_value_ptr, "slopefile", Fnameptr->slopeFile, verbose, mode))
	{
		Parptr->interflowSlopeType = FILE_TYPE;
		return;
	}

	if (read_numeric_param(param_name, line_number, param_value_ptr, "interflow_cs_value", &Parptr->interflow_cs, verbose, mode))
	{
		printf("interflow_cs changed to %" NUM_FMT" \n", Parptr->interflow_cs);
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "interflow_surlag_value", &Parptr->interflow_surlag, verbose, mode))
	{
		printf("interflow_surlag changed to %" NUM_FMT" \n", Parptr->interflow_surlag);
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "interflow_lagindex", &Parptr->interflow_lagindex, verbose, mode))
	{
		printf("interflow_lagindex changed to %" NUM_FMT" \n", Parptr->interflow_lagindex);
		return;
	}

	if (read_numeric_param(param_name, line_number, param_value_ptr, "interflow_t_conc_value", &Parptr->interflow_t_conc, verbose, mode))
	{
		printf("interflow_t_conc changed to %" NUM_FMT" \n", Parptr->interflow_t_conc);
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "interflow_tstep", &Parptr->interflowTstep, verbose, mode))
	{
		printf("interflow_tstep changed to %" NUM_FMT" \n", Parptr->interflowTstep);
		return;
	}
	//-------------------------------interflow_multilayer--------------------------------
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_interflow_multilayer", verbose, mode)) // Enable interflow
	{
		Statesptr->use_interflow_multilayer = ON;
		return;
	}
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_INTERFLOW_CS_VALUE, verbose, mode))
	{
		Statesptr->multi_interflowCsValueOfLyr = ON;
		Parptr->multi_interflowCsValueOfLyr = new NUMERIC_TYPE[Parptr->multi_nSoilLyrs];
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_interflowCsValueOfLyr == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_INTERFLOW_CS_VALUE, strlen(MULTI_INTERFLOW_CS_VALUE)) == 0)
	{
		ReadMultilayerValues(param_name, param_value_ptr, line_number, Parptr->multi_interflowCsValueOfLyr, MULTI_INTERFLOW_CS_VALUE, Parptr, verbose, mode);
		return;
	}



	//-------------------------------percolation_singlelayer--------------------------------
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_percolation_singlelayer", verbose, mode))
	{
		Statesptr->use_percolation_singlelayer = ON;
		return;
	}
	// fieldCapacity
	if (read_string_param(param_name, line_number, param_value_ptr, "pc_field_capacity_file", Fnameptr->fieldCapacityFile, verbose, mode))
	{
		Parptr->useFieldCapacityType = FILE_TYPE;
		return;
	}
	// poreIndex
	if (read_string_param(param_name, line_number, param_value_ptr, "pc_pore_index_file", Fnameptr->poreIndexFile, verbose, mode))
	{
		Parptr->usePoreIndexType = FILE_TYPE;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "pc_pore_index_value", &Parptr->poreIndexValue, verbose, mode))
	{
		Parptr->usePoreIndexType = VALUE_TYPE;
		return;
	}
	//-------------------------------percolation_multilayer--------------------------------
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_percolation_multilayer", verbose, mode))
	{
		Statesptr->use_percolation_multilayer = ON;
		return;
	}
	if (read_integer_param(param_name, line_number, param_value_ptr, "multi_nSoilLyrs", &Parptr->multi_nSoilLyrs, verbose, mode))
	{
		printf("multi_nSoilLyrs changed to %" NUM_FMT" \n", Parptr->multi_nSoilLyrs);
		return;
	}
	// pore index
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_SOIL_POREINDEX_FILE, verbose, mode))
	{
		Statesptr->multi_soilPoreIndexFile = ON;
		Parptr->usePoreIndexType = FILE_TYPE;
		for (int i = 0; i < Parptr->multi_nSoilLyrs; ++i) {
			Fnameptr->multi_soilPoreIndexFile[i] = new char[256]; 
		}
		Parptr->multi_curSoilLyr = 0;
		return;
	}

	if (read_numeric_param(param_name, line_number, param_value_ptr, MULTI_SOIL_POREINDEX_VALUE, &Parptr->poreIndexValue, verbose, mode))
	{
		Parptr->usePoreIndexType = VALUE_TYPE;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, SOIL_POREINDEX_SCALE_FACTOR, &Parptr->poreIndexScaleFactor, verbose, mode))
	{
		return;
	}
	

	if (Statesptr->multi_soilPoreIndexFile == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_SOIL_POREINDEX_FILE, strlen(MULTI_SOIL_POREINDEX_FILE)) == 0)
	{
		ReadMultilayerFiles(param_name, param_value_ptr, line_number, Fnameptr->multi_soilPoreIndexFile, MULTI_SOIL_POREINDEX_FILE, Parptr, verbose, mode);
		return;
	}

	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_SOIL_POREINDEX_VALUE, verbose, mode))
	{
		Statesptr->multi_soilPoreIndexOfLyr = ON;
		Parptr->multi_soilPoreIndexOfLyr = new NUMERIC_TYPE[Parptr->multi_nSoilLyrs];
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_soilPoreIndexOfLyr == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_SOIL_POREINDEX_VALUE, strlen(MULTI_SOIL_POREINDEX_VALUE)) == 0)
	{
		ReadMultilayerValues(param_name, param_value_ptr, line_number, Parptr->multi_soilPoreIndexOfLyr, MULTI_SOIL_POREINDEX_VALUE,  Parptr, verbose, mode);
	...
	// field capacity
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_SOIL_FC_FILE, verbose, mode))
	{
		Statesptr->multi_soilFcFile = ON;
		for (int i = 0; i < Parptr->multi_nSoilLyrs; ++i) {
			Fnameptr->multi_soilFcFile[i] = new char[256];
		}
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_soilFcFile == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_SOIL_FC_FILE, strlen(MULTI_SOIL_FC_FILE)) == 0)
	{
		ReadMultilayerFiles(param_name, param_value_ptr, line_number, Fnameptr->multi_soilFcFile, MULTI_SOIL_FC_FILE, Parptr, verbose, mode);
		return;
	}
	// ks
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_SOIL_KS_FILE, verbose, mode))
	{
		Statesptr->multi_soilKsFile = ON;
		for (int i = 0; i < Parptr->multi_nSoilLyrs; ++i) {
			Fnameptr->multi_soilKsFile[i] = new char[256];
		}
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_soilKsFile == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_SOIL_KS_FILE, strlen(MULTI_SOIL_KS_FILE)) == 0)
	{
		ReadMultilayerFiles(param_name, param_value_ptr, line_number, Fnameptr->multi_soilKsFile, MULTI_SOIL_KS_FILE, Parptr, verbose, mode);
		return;
	}
	// thickness 
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_SOIL_THICKNESS_FILE, verbose, mode))
	{
		Statesptr->multi_soilThicknessFile = ON;
		for (int i = 0; i < Parptr->multi_nSoilLyrs; ++i) {
			Fnameptr->multi_soilThicknessFile[i] = new char[256];
		}
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_soilThicknessFile == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_SOIL_THICKNESS_FILE, strlen(MULTI_SOIL_THICKNESS_FILE)) == 0)
	{
		ReadMultilayerFiles(param_name, param_value_ptr, line_number, Fnameptr->multi_soilThicknessFile, MULTI_SOIL_THICKNESS_FILE, Parptr, verbose, mode);
		return;
	}
	// init soilmoisture 
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_SOIL_INIT_MOIS_FILE, verbose, mode))
	{
		Statesptr->multi_soilInitMoistureFile = ON;
		for (int i = 0; i < Parptr->multi_nSoilLyrs; ++i) {
			Fnameptr->multi_soilInitMoistureFile[i] = new char[256];
		}
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_soilInitMoistureFile == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_SOIL_INIT_MOIS_FILE, strlen(MULTI_SOIL_INIT_MOIS_FILE)) == 0)
	{
		ReadMultilayerFiles(param_name, param_value_ptr, line_number, Fnameptr->multi_soilInitMoistureFile, MULTI_SOIL_INIT_MOIS_FILE, Parptr, verbose, mode);
		return;
	}
	// ksFactorV
	if (Statesptr->use_groundwater) {
		if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_KSFACTOR_VALUE_V, verbose, mode))
		{
			Statesptr->multi_ksFactorValueV = ON;
			Parptr->multi_ksFactorVOfLyr = new NUMERIC_TYPE[Parptr->multi_nSoilLyrs];
			Parptr->multi_curSoilLyr = 0;
			return;
		}
		if (Statesptr->multi_ksFactorValueV == ON && Parptr->multi_curSoilLyr < (Parptr->multi_nSoilLyrs) && strncmp(param_name, MULTI_KSFACTOR_VALUE_V, strlen(MULTI_KSFACTOR_VALUE_V)) == 0)
		{
			ReadMultilayerValues(param_name, param_value_ptr, line_number, Parptr->multi_ksFactorVOfLyr, MULTI_KSFACTOR_VALUE_V, Parptr, verbose, mode);
			return;
		}
	}
	else {
		...
		{
			Statesptr->multi_ksFactorValueV = ON;
			Parptr->multi_ksFactorVOfLyr = new NUMERIC_TYPE[Parptr->multi_nSoilLyrs - 1];
			Parptr->multi_curSoilLyr = 0;
			return;
		}
		if (Statesptr->multi_ksFactorValueV == ON && Parptr->multi_curSoilLyr < (Parptr->multi_nSoilLyrs - 1) && strncmp(param_name, MULTI_KSFACTOR_VALUE_V, strlen(MULTI_KSFACTOR_VALUE_V)) == 0)
		{
			ReadMultilayerValues(param_name, param_value_ptr, line_number, Parptr->multi_ksFactorVOfLyr, MULTI_KSFACTOR_VALUE_V, Parptr, verbose, mode);
			return;
		}
	...
	// ksFactorH
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_KSFACTOR_VALUE_H, verbose, mode))
	{
		Statesptr->multi_ksFactorValueH = ON;
		Parptr->multi_ksFactorHOfLyr = new NUMERIC_TYPE[Parptr->multi_nSoilLyrs];
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_ksFactorValueH == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_KSFACTOR_VALUE_H, strlen(MULTI_KSFACTOR_VALUE_H)) == 0)
	{
		ReadMultilayerValues(param_name, param_value_ptr, line_number, Parptr->multi_ksFactorHOfLyr, MULTI_KSFACTOR_VALUE_H, Parptr, verbose, mode);
		return;
	}

	if (read_numeric_param(param_name, line_number, param_value_ptr, MULTI_KSFACTOR_INFIL_VALUE, &Parptr->ksFactorInfil, verbose, mode))
	{
		printf("ksFactorValueInfil changed to %" NUM_FMT" \n", Parptr->ksFactorInfil);
		return;
	}
	if (read_empty_param(param_name, line_number, param_value_ptr, MULTI_SOIL_POROSITY_FILE, verbose, mode))
	{
		Statesptr->multi_soilPorosityFile = ON;
		for (int i = 0; i < Parptr->multi_nSoilLyrs; ++i) {
			Fnameptr->multi_soilPorosityFile[i] = new char[256];
		}
		Parptr->multi_curSoilLyr = 0;
		return;
	}
	if (Statesptr->multi_soilPorosityFile == ON && Parptr->multi_curSoilLyr < Parptr->multi_nSoilLyrs && strncmp(param_name, MULTI_SOIL_POROSITY_FILE, strlen(MULTI_SOIL_POROSITY_FILE)) == 0)
	{
		ReadMultilayerFiles(param_name, param_value_ptr, line_number, Fnameptr->multi_soilPorosityFile, MULTI_SOIL_POROSITY_FILE, Parptr, verbose, mode);
		return;
	}
	//-------------------------------SEIMS AET_PTH--------------------------------
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_seims_aet", verbose, mode))
	{
		Statesptr->use_seims_aet = ON;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "esco", &Parptr->esco, verbose, mode))
	{
		return;
	}
	if (read_integer_param(param_name, line_number, param_value_ptr, "multi_nSoilEvapLyrs", &Parptr->multi_nSoilEvapLyrs, verbose, mode))
	{
		if (verbose == ON) printf("ET soil layers(multi_nSoilEvapLyrs) changed to %" NUM_FMT" \n", Parptr->multi_nSoilEvapLyrs);
		return;
	}
	...
	if (read_string_param(param_name, line_number, param_value_ptr, RUNOFF_CO_FILE, Fnameptr->runoffCoFile, verbose, mode))
	{
		Parptr->useRunoffCoType = FILE_TYPE;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, RUNOFF_CO_VAL, &Parptr->runoffCoVal, verbose, mode))
	{
		Parptr->useRunoffCoType = VALUE_TYPE;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "kRun", &Parptr->kRun, verbose, mode))
	{
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "pMax", &Parptr->pMax, verbose, mode))
	{
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, ALPHA, &Parptr->alpha, verbose, mode))
	{
		Parptr->useAlphaType = VALUE_TYPE;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, RUNOFF_CO_FACTOR, &Parptr->runoffCoFactor, verbose, mode))
	{
		return;
	}
	//-------------------------------XAJ ET--------------------------------
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_xaj_evap", verbose, mode))
	{
		Statesptr->use_xaj_evap = ON;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "c", &Parptr->c, verbose, mode))
	{
	...
	{
		Statesptr->use_dhsvm = ON;
		return;
	}
	if (read_string_param(param_name, line_number, param_value_ptr, "soilThicknessAllLyrs", Fnameptr->soilThicknessAllLyrsFile, verbose, mode))
	{
		Parptr->useSoilThicknessAllLyrsType = FILE_TYPE;
		return;
	}
	if (read_string_param(param_name, line_number, param_value_ptr, "ksLatFile", Fnameptr->ksLatFile, verbose, mode))
	{
		Parptr->useKsLatType = FILE_TYPE;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "ksLatValue", &Parptr->ksLatValue, verbose, mode))
	{
		Parptr->useKsLatType = VALUE_TYPE;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "KsLatExpValue", &Parptr->KsLatExpValue, verbose, mode))
	{
		printf("KsLatExpValue changed to %" NUM_FMT" \n", Parptr->KsLatExpValue);
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "soilWaterDepthThresh", &Parptr->soilWaterDepthThresh, verbose, mode))
	{
		return;
	}
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_change_acccum_depth", verbose, mode))
	{
		Statesptr->use_change_acccum_depth = ON;
		return;
	}

	//-------------------------------groundwater--------------------------------
	if (read_empty_param(param_name, line_number, param_value_ptr, "use_groundwater", verbose, mode))
	{
		Statesptr->use_groundwater = ON;
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "gw_recession_coefficient", &Parptr->recessionCoefficient, verbose, mode))
	{
		printf("gw_recession_coefficient changed to %" NUM_FMT" \n", Parptr->recessionCoefficient);
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "gw_recession_exponent", &Parptr->recessionExponent, verbose, mode))
	{
		printf("gw_recession_exponent changed to %" NUM_FMT" \n", Parptr->recessionExponent);
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "gw_deep_coefficient", &Parptr->deepCoefficient, verbose, mode))
	{
		printf("gw_deep_coefficient changed to %" NUM_FMT" \n", Parptr->deepCoefficient);
		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "gw_init_gw_storage", &Parptr->initGwStorageDepth, verbose, mode))
	{
		printf("gw_init_gw_storage changed to %" NUM_FMT" \n", Parptr->initGwStorageDepth);
		return;
	}

	if (read_numeric_param(param_name, line_number, param_value_ptr, "gw_storage_depth_max", &Parptr->GwStorageDepthMax, verbose, mode))
	{
		printf("gw_storage_depth_max changed to %" NUM_FMT" \n", Parptr->GwStorageDepthMax);
		return;
	}

	if (read_numeric_param(param_name, line_number, param_value_ptr, "gw_tstep", &Parptr->gwTstep, verbose, mode))
	{
		printf("gw_tstep changed to %" NUM_FMT" \n", Parptr->gwTstep);
		return;
	}

	// xdw modify, support saturation-excess infiltration
	if (read_numeric_param(param_name, line_number, param_value_ptr, "saturation_value", &Parptr->saturation_value, verbose, mode))
	{
		printf("saturation_value changed to %" NUM_FMT" \n", Parptr->saturation_value);
		return;
	}

	
	// xdw modify, support glacier and snow melt
	if (read_string_param(param_name, line_number, param_value_ptr, "glacier_file", Fnameptr->glacierFile, verbose, mode))
	{
		// 在读取冰川范围raster的基础上，才支持以glacier_value指定冰川厚度
		Parptr->useGlacierType = FILE_TYPE;
		Statesptr->use_snow_glacier = ON;

		return;
	}
	if (read_numeric_param(param_name, line_number, param_value_ptr, "glacier_value", &Parptr->glacierValue, verbose, mode))
	{
		Parptr->useGlacierType = FILE_AND_VALUE_TYPE;
		return;
	}
	if (read_string_param(param_name, line_number, param_value_ptr, "snow_file", Fnameptr->snowFile, verbose, mode))
	{
		// 在读取积雪范围raster的基础上，才支持以snow_value指定积雪厚度
		Parptr->useSnowType = FILE_TYPE;
		Statesptr->use_snow_glacier = ON;
		if (read_numeric_param(param_name, line_number, param_value_ptr, "snow_value", &Parptr->snowValue, verbose, mode))
		{
			Parptr->useSnowType = FILE_AND_VALUE_TYPE;
		}
		return;
	}

	// 积雪度日因子
	if (read_numeric_param(param_name, line_number, param_value_ptr, "FddSnow", &Parptr->FddSnow, verbose, mode))
	{
		printf("FddSnow changed to %" NUM_FMT" \n", Parptr->FddSnow);
		return;
	}
	// 冰川度日因子
	if (read_numeric_param(param_name, line_number, param_value_ptr, "FddGlacier", &Parptr->FddGlacier, verbose, mode))
	{
		printf("FddGlacier changed to %" NUM_FMT" \n", Parptr->FddGlacier);
		return;
	}
	// 度日因子再冻结修正参数
	if (read_numeric_param(param_name, line_number, param_value_ptr, "Frr", &Parptr->Frr, verbose, mode))
	{
		printf("Frr changed to %" NUM_FMT" \n", Parptr->Frr);
		return;
	}

	// xdw add, support temperature for glacier and snow melt
	if (read_string_param(param_name, line_number, param_value_ptr, "temperature_file", Fnameptr->temperatureFile, verbose, mode))
	{
		Statesptr->use_temperature = ON;
		return;
	}
	if (read_string_param(param_name, line_number, param_value_ptr, "temperature_csv", Fnameptr->temperatureCsvFile, verbose, mode))
	{
		Statesptr->use_temperature = ON;
		return;
	}
	// 冰雪融化温度阈值
	if (read_numeric_param(param_name, line_number, param_value_ptr, "melt-temperature", &Parptr->melt_temperature, verbose, mode))
	{
		printf("Melt temperature changed to %.5" NUM_FMT" \n", Parptr->melt_temperature);
		return;
	}
	...
	if (read_string_param(param_name, line_number, param_value_ptr, "rainfall_csv", Fnameptr->rain_csvfilename, verbose, mode))
	{
		Statesptr->rainfall = ON;
		return;
	}
	...
	// xiaodw add,Enable distributed rainfall
	if (read_string_param(param_name, line_number, param_value_ptr, "rain_tif_folder", Fnameptr->rainTifFolder, verbose, mode))
	{
		return;
	}
	...
	if (read_string_param(param_name, line_number, param_value_ptr, "rain_end_time", Solverptr->rain_end_time, verbose, mode))
	{
		return;
	}
	if (read_integer_param(param_name, line_number, param_value_ptr, "rain_time_step", &Solverptr->rain_time_step, verbose, mode))
	{
	...
	if (read_empty_param(param_name, line_number, param_value_ptr, "limitslopes", verbose, mode))
    {
        if (Statesptr->dg2 == OFF)
        {
			if (Statesptr->fv2 == OFF) {
				printf("ERROR Slope limiter only applicable to DG2 and FV2 solvers\n");
				exit(EXIT_FAILURE);
			}
        }
...
void ReadMultilayerValues(char* param_name, char* param_value_ptr, int line_number, NUMERIC_TYPE*  multi_Values,
	...
	stringstream ss;
	ss << param_prefix << "_" << Parptr->multi_curSoilLyr;
	string fileName = ss.str();
	char* value_ptr = new char[fileName.length() + 1];
	strcpy(value_ptr, fileName.c_str());
	if (read_numeric_param(param_name, line_number, param_value_ptr, value_ptr, &multi_Values[Parptr->multi_curSoilLyr], verbose, mode))
	{
		//printf("multi_soilPoreIndexOfLyr added %" NUM_FMT" \n", Parptr->multi_soilPoreIndexOfLyr[Parptr->multi_curSoilLyr]);
		delete[] value_ptr;
		Parptr->multi_curSoilLyr++;
		return;
	}
}
...
void ReadMultilayerFiles(char* param_name, char* param_value_ptr, int line_number, char*  multi_Files[],
	string param_prefix,  Pars *Parptr, int verbose, int mode) {
	// 读取文件名为multi_soilPoreIndexFile_1,2,3...这样的文件
	stringstream ss;
	ss << param_prefix << "_" << Parptr->multi_curSoilLyr;
	string fileName = ss.str();
	char* fileName_ptr = new char[fileName.length() + 1];
	strcpy(fileName_ptr, fileName.c_str());
	if (read_string_param(param_name, line_number, param_value_ptr, fileName_ptr, multi_Files[Parptr->multi_curSoilLyr], verbose, mode))
	{
		//printf("%s added %s \n", param_name,multi_Files[Parptr->multi_curSoilLyr]);
		delete[] fileName_ptr;
		Parptr->multi_curSoilLyr++;
		return;
	}
