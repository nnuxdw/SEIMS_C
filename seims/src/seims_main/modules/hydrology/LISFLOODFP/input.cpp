...
void LoadStages(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Stage *Locptr, const int verbose)
{
	...
	Locptr->stage_loc_x = memory_allocate_numeric_legacy(Locptr->Nstages);
	Locptr->stage_loc_y = memory_allocate_numeric_legacy(Locptr->Nstages);
...
void LoadWeir(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, const int verbose)
{
	...
	fgets(buff, LINE_BUFFER_LEN, fp);
	sscanf(buff, "%i", &nw);

	Arrptr->weir_count = nw;

	Arrptr->Weir_hc = memory_allocate_numeric_legacy(nw);
	Arrptr->Weir_Cd = memory_allocate_numeric_legacy(nw);
	Arrptr->Weir_m = memory_allocate_numeric_legacy(nw);
	Arrptr->Weir_w = memory_allocate_numeric_legacy(nw);
	Arrptr->Weir_Typ = new EWeirType[nw]; // type of structure... weir = 0, bridge = 1;

	Arrptr->Weir_Fixdir = new EDirection[nw];   // Fixed flow directions
...
void LoadRiverNetwork(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, vector<ChannelSegmentType> *ChannelSegmentsVecPtr, Arrays *Arrptr, vector<QID7_Store> *QID7_Vec_Ptr, vector<int> *RiversIndexVecPtr, const int verbose)
{
...
void LoadRiver(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, vector<ChannelSegmentType> *ChannelSegmentsVecPtr, Arrays *Arrptr, vector<QID7_Store> *QID7_Vec_Ptr, vector<int> *RiversIndexVecPtr, const int verbose)
{
	...
	for (chseg = 0; chseg < tmp_int; chseg++) // CCS Slight reorganisation of old LoadRiver function but fundamentally unchanged.
	{
		...
		if (Statesptr->chainagecalc == ON)
		{
			...
			// multi_adjustPD each dx by ratio calculated previously
			for (i = 0; i < npoints; i++)
			{
				for (j = 0; j < csp->chsz; j++)
				{
					...
					{
						csp->Chandx[j] = csp->Chandx[j] / rp[i + 1]; // multi_adjustPD by ratio calculated previously
					}
				}
			}
			...
			// also multi_adjustPD original xs chainage as this was based on centre cell distance
...
// 读取manningfile，并给nodata处赋值为fpfric常量
...
// 读取SGCmanningfile，并给nodata处赋值为SGC_n常量
...
void LoadPor(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, const int verbose)
{
	...
	else if (Parptr->Por_Ident == 2)
	{
		...
		Arrptr->paerial = memory_allocate_numeric_legacy(Parptr->xsz*Parptr->ysz*Parptr->zsz);
	...
	else if (Parptr->Por_Ident == 4)
	{
		...
		Arrptr->paerial = memory_allocate_numeric_legacy(Parptr->xsz*Parptr->ysz*Parptr->zsz);
		Arrptr->pbound = memory_allocate_numeric_legacy(Parptr->xsz*Parptr->ysz*Parptr->zsz * 4);
...
void LoadDEM(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, const int verbose)
{
	...
	if (Statesptr->Roe == ON)
	{
		...
		Arrptr->LSHU = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->RSHU = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->BSHU = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->TSHU = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->LSHV = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->RSHV = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->BSHV = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->TSHV = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
	...
	Arrptr->maxH = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
	Arrptr->totalHtm = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
	Arrptr->Qx = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	Arrptr->Qy = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	// added to record Hflow (needed for velocity and hazard calculations
	//Arrptr->Hflowx=memory_allocate_zero_numeric_legacy((Parptr->xsz+1)*(Parptr->ysz+1));
	//Arrptr->Hflowy=memory_allocate_zero_numeric_legacy((Parptr->xsz+1)*(Parptr->ysz+1));

	if (Statesptr->voutput == ON)
	{
		Arrptr->Vx = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
		Arrptr->Vy = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
		Arrptr->maxVx = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
		Arrptr->maxVy = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	}
	if (Statesptr->hazard == ON)
	{
		Arrptr->maxVc = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->maxVcH = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->maxHaz = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
	}
	if (Statesptr->SGC == ON)
	{
		// Geometric variables
		Arrptr->SGCwidth = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->SGCz = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->SGCc = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);

		// Flow variables
		Arrptr->QxSGold = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
		Arrptr->QySGold = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
		// Bank full variables
		Arrptr->SGCbfH = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->SGCbfV = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		// Volume variables
		Arrptr->SGCVol = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		Arrptr->SGCdVol = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		// Model parameters
		Arrptr->SGCgroup = new int[Parptr->xsz*Parptr->ysz]();
		if (Statesptr->save_Qs == ON)
		{
			Arrptr->SGCFlowWidth = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		}
	}

	Arrptr->Qxold = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	Arrptr->Qyold = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	...
	Arrptr->U = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	//Arrptr->V=memory_allocate_zero_numeric_legacy((Parptr->xsz+1)*(Parptr->ysz+1));

	// allocate memory for none zero arrays
	Arrptr->maxHtm = memory_allocate_numeric_legacy(Parptr->xsz*Parptr->ysz);
	Arrptr->initHtm = memory_allocate_numeric_legacy(Parptr->xsz*Parptr->ysz);
	Arrptr->TRecx = memory_allocate_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	Arrptr->TRecy = memory_allocate_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	Arrptr->LimQx = memory_allocate_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	Arrptr->LimQy = memory_allocate_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
	...
	Arrptr->dx = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
	Arrptr->dy = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
	Arrptr->dA = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
...
void LoadPOIs(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Pois *Poisptr, const int verbose) {
	...
	char buff[800];
	NUMERIC_TYPE px, py;
	int pi = -1, maxpi = 20000;
	if (strlen(Fnameptr->poifilename) == 0)
	{
		if (verbose == ON) printf("No poi used\n");
		return;
	}
	fp = fopen_or_die(Fnameptr->poifilename, "rb", "Loading POIs......\n", verbose);
	fscanf(fp, "%i", &Poisptr->num);
	Poisptr->xpi = new int[Poisptr->num];
	Poisptr->ypi = new int[Poisptr->num];
	while (!feof(fp))
	{
		fscanf(fp, "%s", buff);
		if (feof(fp)) break;
...
void LoadBCs(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, BoundCs *BCptr, const int verbose)
{
	...
	while (!feof(fp))
	{
		...
		else if (!STRCMPi(buff, "HFIX") && BCptr->xpi[pi] > -1 && pi > -1)
		{
			...
			//BCptr->BC_Val[pi] = BC_tmp;
			//BCptr->BC_Ident[pi] = HFIX2;
		}
		...
		while (fgets(line_buffer, LINE_BUFFER_LEN, fp))
		{
			...
			*/
			...
			if (timeSeriesUsed == 0)
			{
				...
				//FreeTimeSeries(timeSeries); // JAMES SHAW VERSION
...
void LoadDataByCSV(char *filename,  TimeSeries ** dataSeries, const int verbose, int col_index) {
	...
	series->count = line_count;
	series->prev_index = 0;
	series->prev_time = C(-1.0);
	series->time = (NUMERIC_TYPE*)memory_allocate(series->count * sizeof(NUMERIC_TYPE));
	series->value = (NUMERIC_TYPE*)memory_allocate(series->count * sizeof(NUMERIC_TYPE));
	file.open(filename);
	line_count = 0;
	time_t first_time;
	while (getline(file, line))
	{
		...
		string timeStr;
		struct tm tm;
		//NUMERIC_TYPE air_temperature, rainfall;
		stringstream ss(line);
		string token;
		getline(ss, token, ',');
		sscanf(token.c_str(), "%d/%d/%d %d:%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min);
		tm.tm_sec = 0;
		//sscanf(line.c_str(), "%s,%*[^,],%*[^,],%*[^,],%*[^,],%*[^,],%*[^,],%f,%*[^,],%f", &timeStr, &air_temperature, &rainfall);
		//sscanf(timeStr.c_str(), "%d/%d/%d %d:%d:%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min, &tm.tm_sec);
		tm.tm_year -= 1900;
		tm.tm_mon -= 1;
		time_t current_time;
		// first time
		if (line_count == 1)
		{
			first_time = mktime(&tm);
			current_time = mktime(&tm);
			series->time[0] = difftime(current_time, first_time);
		}
		else
		{
			current_time = mktime(&tm);
			series->time[line_count - 1] = difftime(current_time, first_time);
		}
		for (int i = 1; i < col_index - 1; i++)
		{
			getline(ss, token, ',');
		}
		getline(ss, token, ',');
...
// xdw add，support read rain, temperature data from meteorological station csv data directly
void LoadMeteroByCSV(Fnames *Fnameptr, Arrays *Arrptr, const int verbose) {
	//string filename = "G:\\program\\longxiazailongba\\data\\metero\\tgl-glacier-end-220801-2306.csv";
	ifstream file(Fnameptr->rain_csvfilename);
	string line;
	TimeSeries * rainSeries = new TimeSeries();

	TimeSeries * temperatureSeries = new TimeSeries();
	int line_count = 0;
	while (getline(file, line))
	{
		if (line_count == 0)
	...
	file.close();
	// initialize rainSeries
	rainSeries->count = line_count;
	rainSeries->prev_index = 0;
	rainSeries->prev_time = C(-1.0);
	rainSeries->time = (NUMERIC_TYPE*)memory_allocate(rainSeries->count * sizeof(NUMERIC_TYPE));
	rainSeries->value = (NUMERIC_TYPE*)memory_allocate(rainSeries->count * sizeof(NUMERIC_TYPE));
	// initialize temperatureSeries
	temperatureSeries->count = line_count;
	temperatureSeries->prev_index = 0;
	temperatureSeries->prev_time = C(-1.0);
	temperatureSeries->time = (NUMERIC_TYPE*)memory_allocate(temperatureSeries->count * sizeof(NUMERIC_TYPE));
	temperatureSeries->value = (NUMERIC_TYPE*)memory_allocate(temperatureSeries->count * sizeof(NUMERIC_TYPE));

	file.open(Fnameptr->rain_csvfilename);
	line_count = 0;
	time_t first_time;
	while (getline(file, line))
	{
		...
		if (line_count == 0)
		{
			...
			sscanf(line.c_str(), "%d/%d/%d %d:%d:%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min, &tm.tm_sec);
			tm.tm_year -= 1900;
			tm.tm_mon -= 1;
			first_time = mktime(&tm);
			line_count++;
		...
		NUMERIC_TYPE time, air_temperature, rainfall;
		sscanf(line.c_str(), "%f,%*[^,],%*[^,],%*[^,],%*[^,],%*[^,],%*[^,],%f,%*[^,],%f", &time, &air_temperature, &rainfall);
		struct tm tm;
		//strptime(line.c_str(), "%Y/%m/%d %H:%M:%S", &tm);
		time_t current_time = mktime(&tm);
		rainSeries->time[line_count - 1] = difftime(current_time, first_time);
		rainSeries->value[line_count - 1] = rainfall;
		temperatureSeries->time[line_count - 1] = difftime(current_time, first_time);
		temperatureSeries->value[line_count - 1] = air_temperature;
		//time_data[line_count - 1] = difftime(current_time, first_time);
		//air_temperature_data[line_count - 1] = air_temperature;
		//rainfall_data[line_count - 1] = rainfall;
		line_count++;
	}
	Arrptr->rain = rainSeries;
	Arrptr->temperature = temperatureSeries;
	file.close();
	return;
}
//-----------------------------------------------------------------------------
// LOAD RAINFALL DISTRIBUTION MASK
// xiaodw comment, use dem.asc as rain mask
void LoadRainmask(Fnames *Fnameptr, Pars *Parptr, Arrays *Arrptr, States *Statesptr, const int verbose)
{
	...
	Arrptr->Rainmask = memory_allocate_numeric_legacy(Parptr->xsz*Parptr->ysz);
...
void LoadSGC(Fnames *Fnameptr, Pars *Parptr, Arrays *Arrptr, States *Statesptr, const int verbose)
{
	...
	// 如果par中SGCbank未指定，则将Arrptr->SGCz赋值为dem
	...
	else
	{
		...
		for (j = 0; j < Parptr->ysz; j++)
		{
			for (i = 0; i < Parptr->xsz; i++)
			{
				...
				// 在SGCbank的nodata区域，也赋值为dem。因此SGCz其实是河道高程，不是河道深度
...
void LoadTimeVaringTemperature(Fnames *Fnameptr, Arrays *Arrptr, const int verbose)
{
	...
	fp = fopen_or_die(Fnameptr->temperatureFile, "r", "Loading time varying temperature", verbose);

	Arrptr->temperature = LoadTimeSeries(Fnameptr->temperatureFile, fp, ON);
	fclose(fp);
...
void initialize_array(float * arr,  const int grid_rows,const int grid_cols, const int grid_cols_padded) {
	if (arr == NULL)
	{
		...
		//int padding = sizeof(float) * grid_cols_padded;
		//for (int j = 0; j < grid_rows; j++) {
		//	int dest_row_index = j * grid_cols_padded;
		//	memset(arr + dest_row_index, 0, sizeof(float) * grid_cols_padded);
		//	memset(arr + dest_row_index + grid_cols, -1, padding);
		//}

		arr = new float[grid_cols_padded * grid_rows];
		for (int i = 0; i < grid_rows; i++)
		{
			for (int j = 0; j < grid_cols; j++)
			{
				arr[i*grid_cols + j] = 0.0;
			}
			for (int j = grid_cols; j < grid_cols_padded; j++)
			{
				arr[i*grid_cols + j] = -1.0;
			}
		}
	}
}
void loadGlacierSnowProperties(Fnames *Fnameptr, Pars *Parptr, int verbose) {
	int m_nCells = Parptr->xsz * Parptr->ysz;
	// glacier thickness
	Parptr->glacier = new NUMERIC_TYPE[m_nCells];
	LoadProperty(Parptr->useGlacierType, "glacier_thickness", Parptr->glacier, Fnameptr->glacierFile, Parptr->glacierValue, m_nCells);
	// snow thickness
	Parptr->snow = new NUMERIC_TYPE[m_nCells];
	LoadProperty(Parptr->useSnowType, "snow_thickness", Parptr->snow, Fnameptr->snowFile, Parptr->snowValue, m_nCells);
	//for (int i = 0; i < Parptr->xsz * Parptr->ysz; i++)
	//{
	//	if (Parptr->snow[i] > 0.1)
	//	{
	//		cout << "snow[" << i << "]:" << Parptr->snow[i] << i/ Parptr->xsz << "," << i % Parptr->xsz << endl;
	//	}
	//	if (Parptr->glacier[i] > 0.1)
	//	{
	//		cout << "glacier[" << i << "]:" << Parptr->glacier[i] << i / Parptr->xsz << "," << i % Parptr->xsz << endl;
	//	}
	//}
	//cout << endl;
}
...
void loadSoilPropertiesGW(Fnames *Fnameptr, Pars *Parptr, int m_nCells) {
	...
	LoadProperty(Parptr->useFieldCapacityType, "field_capacity", Parptr->fieldCapacity, Fnameptr->fieldCapacityFile, 0.0, m_nCells);
	// poreIndex
	//Parptr->poreIndex = new NUMERIC_TYPE[m_nCells];
	LoadProperty(Parptr->usePoreIndexType, "pore_index", Parptr->poreIndex, Fnameptr->poreIndexFile, Parptr->poreIndexValue, m_nCells);
}
...
void loadSoilPropertiesInterflow(Fnames *Fnameptr, Pars *Parptr, int m_nCells) {
	...
	LoadProperty(Parptr->interflowSlopeType, "slope", Parptr->slope, Fnameptr->slopeFile, 0.0, m_nCells);
}
...
void loadSoilPropertiesPerco_Multilayer(Fnames *Fnameptr, Pars *Parptr, States *Statesptr, int m_nCells) {
	for (int i = 0; i < Parptr->multi_nSoilLyrs; i++)
	{
		...
		{
			LoadProperty(FILE_TYPE, "multi_soilFc", Parptr->multi_soilFc[i], Fnameptr->multi_soilFcFile[i], 0.0, m_nCells);
		}
		if (Statesptr->multi_soilPorosityFile == ON)
		{
			LoadProperty(FILE_TYPE, "multi_soilPorosity", Parptr->multi_soilPorosity[i], Fnameptr->multi_soilPorosityFile[i], 0.0, m_nCells);
		}
		if (Statesptr->multi_soilKsFile == ON)
		{
			LoadProperty(FILE_TYPE, "multi_soilKs", Parptr->multi_soilKs[i], Fnameptr->multi_soilKsFile[i], 0.0, m_nCells);
		}
		if (Statesptr->multi_soilThicknessFile == ON)
		{
			LoadProperty(FILE_TYPE, "multi_soilThickness", Parptr->multi_soilThickness[i], Fnameptr->multi_soilThicknessFile[i], 0.0, m_nCells);
		}
...
void loadSoilPropertiesGASinglelayer(Fnames *Fnameptr,Pars *Parptr,int m_nCells) {
	...
	LoadProperty(Parptr->useFieldCapacityType, "field_capacity", Parptr->fieldCapacity, Fnameptr->fieldCapacityFile, 0.0, m_nCells);
	//if (Parptr -> useKsType == FILE_TYPE)
	//{
	//	char * ksFile = Fnameptr -> ksFile;
	//	readTIFByGdal(ksFile, Parptr->ks);
	//}
	//else if (Parptr->useKsType == VALUE_TYPE)
	//{
	//	for (int i = 0; i < m_nCells; ++i) {
	//		Parptr->ks[i] = Parptr ->ksValue;
	//	}
	//}
	//else
	//{
	...
	//char * clayFile = "F:\\program\\lisflood\\data\\liujiahe\\dem.tif";
	//char * sandFile = "F:\\program\\lisflood\\data\\liujiahe\\dem.tif";
	//char * rootDepth = "D:\\lzu\\courses\\atmosphericRemoteSensing\\2008-2018_TIF_O3_DATA\\2008\\2008m0102.tif";

	//int m_nCells = Parptr->xsz * Parptr->ysz;
	//Parptr->ks = new float[m_nCells];
	//Parptr->initSoilMoisture = new float[m_nCells];
	//Parptr->porosity = new float[m_nCells];
	//Parptr->clay = new float[m_nCells];
	//Parptr->sand = new float[m_nCells];
	//Parptr->rootDepth = new float[m_nCells];
	//readTIFByGdal(ksFile, Parptr ->ks);
...
void loadSoilPropertiesDHSVM(Fnames *Fnameptr, Pars *Parptr, int m_nCells) {
	Parptr->soilThicknessAllLyrs = new NUMERIC_TYPE[m_nCells];
	LoadProperty(Parptr->useSoilThicknessAllLyrsType, "soilThicknessAllLyrs", Parptr->soilThicknessAllLyrs, Fnameptr->soilThicknessAllLyrsFile, 0, m_nCells);

	Parptr->ksLat = new NUMERIC_TYPE[m_nCells];
...
void LoadProperty(int type,string paramName, NUMERIC_TYPE * paramPtr, char * filename, NUMERIC_TYPE value, int m_nCells) {
	...
	if (type == FILE_TYPE)
	{
		...
		//Size size;
		//DATA data;
		//readTif("G:\\program\\longxiazailongba\\data\\test\\test.tif", ifh, pde, size, data);
		//readTif(filename, ifh,pde, size, data);
	...
	// xdw add, for flacier and snow melt, support specify a value for the glacier thickness
	else if (type == FILE_AND_VALUE_TYPE)
	{
		//IFH ifh;
		//PDE pde;
		//Size size;
	...
	{
		printf("error load tif file, please config %s file in par file !", paramName.c_str(), paramName.c_str());
	}
}
