...
void CalcSGCz(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, SGCprams *SGCptr, const int verbose)
{
	...
	for (j = 0; j<Parptr->ysz; j++) for (i = 0; i<Parptr->xsz; i++) // loop through model domain
	{
		...
		if (w0 > C(0.0) && Arrptr->DEM[p0] != DEM_NO_DATA) // if there is a sub-grid cahnnel
		{
			...
			// 如果SGCwidth<最大边长，且SGCwidth*河道长度>栅格面积，令河道长度=栅格面积/SGCwidth,防止河道长度太大
			...
			// SGCbfH是河道的深度，即DEM高程-河道底部高程
			// DEM是从dembnk.tif读取，SGCz是从bed.asc读取，两者的差被看作是SGCbfH
...
#pragma omp parallel for private(i,p0,p1,pq0,z0,z1,zb0,zb1,h0,h1,fn,w0,w1,dh,Sf,hflow) 
	for (j = 0; j<Parptr->ysz; j++)
	{
		...
		{
			p0 = i + j*Parptr->xsz;                      // 当前栅格单元下标
			p1 = i + 1 + j*Parptr->xsz;               // 右邻栅格单元下标
			pq0 = i + j*(Parptr->xsz + 1) + 1;    // 下邻栅格单元下标

			z0 = Arrptr->DEM[p0];					// 当前栅格单元高程
			z1 = Arrptr->DEM[p1];
			zb0 = Arrptr->SGCz[p0];
			zb1 = Arrptr->SGCz[p1];
			h0 = Arrptr->H[p0];                        // 当前栅格单元水深？
			h1 = Arrptr->H[p1];
			w0 = Arrptr->SGCwidth[p0];          // 当前栅格单元河道宽度
			w1 = Arrptr->SGCwidth[p1];
			// 如果在河道里，曼宁系数=(当前单元曼宁+右侧单元曼宁)*0.5
