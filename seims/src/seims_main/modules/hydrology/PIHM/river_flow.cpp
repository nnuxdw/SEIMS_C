...
double OvlFlowElemToRiver(const river_struct *river_ptr, elem_struct *bank)
{
	...
	double          bank_h;
	double          river_h;
	// z_bank=河堤高程>侧边三角形高程？河堤高程 : 三角形高程
	...
	//三角形水面高程>河道水面高程，且地表水深高出洼地深度，三角形补给河道
	...
	const river_bc_struct *bc, const river_wstate_struct *ws)
{
	...
	case OUTLET_DIRICHLET:
		// Dirichlet boundary condition 河道水位+河道底部高程 - bc文件中指定的出水口水头
	...
	case OUTLET_NEUMANN:
		...
		break;
	case ZERO_DPTH_GRAD:
		// Zero-depth-gradient boundary conditions  河堤高度+河道底部高程 - bc文件中指定的出水口水头
		distance = 0.5 * shp->length;
...
double ChannelFlowElemToRiver(double effk, double distance, const river_struct *river_ptr, elem_struct *bank)
{
	...
	// 达西定律：流量 = 河段长度 * 平均饱和水力传导度 * 水力梯度 * 平均水深
