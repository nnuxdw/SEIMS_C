...
int main(int argc, char *argv[])
{
	...
	pihm->ptime_calculator = &time_calculator;
	init_time_struct(pihm->ptime_calculator);
#endif
...
#if defined(_STATISTIC_TIME_)
	// xdw, calculate time
	pihm->ptime_calculator->t1 = clock();
	pihm->ptime_calculator->init_time = ((double)(pihm->ptime_calculator->t1 - start_time)) / CLOCKS_PER_SEC;
#endif
	...
	if (spinup_mode)
	{
		...
		pihm->ptime_calculator->spinup_time = ((double)(pihm->ptime_calculator->t2 - pihm->ptime_calculator->t1)) / CLOCKS_PER_SEC;
#endif
	}
	else
	{
