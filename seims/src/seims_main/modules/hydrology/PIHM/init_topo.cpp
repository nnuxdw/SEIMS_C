...
#ifdef USE_MEAN_ELEV
		elem[i].topo.zmax = meshtbl->mean_elev_max[i];
		elem[i].topo.zmin = meshtbl->mean_elev_min[i];
#else
		// 三角形的高程 = 三个顶点的高程平均值
...
#endif
		// 3条边的边长 edge i 就是 node i-1 到 node i+1
...
void CalcSlopeAspect(const meshtbl_struct *meshtbl, elem_struct elem[])
{
	...
	for (i = 0; i < nelem; i++)
	{
		...
		// Calculate slope
		c = sqrt(normal_vector[XCOMP] * normal_vector[XCOMP] + normal_vector[YCOMP] * normal_vector[YCOMP]);
		...
		else
		{
			...
			elem[i].topo.aspect = (se < 0.0) ? 360.0 - elem[i].topo.aspect : elem[i].topo.aspect;
		...
		for (j = 0; j < nelem; j++)
		{
			for (k = 0; k < NUM_EDGE; k++)
			{
				...
				h = (vector[ZCOMP] <= 0.0) ? 90.0 : atan(c / vector[ZCOMP]) * 180.0 / PI;
				...
				if (fabs(phi1 - phi2) > 180.0)
				{
					...
					{
						elem[i].topo.h_phi[ind] = MIN(elem[i].topo.h_phi[ind], h);
					}
