...
#pragma omp parallel for
    ...
    for (int i = 0; i < m_nCells; i++) {
        m_cellSed[i][0] =0.f;
        if (m_sedYield[i] <= 0.f) continue;

        m_cellSed[i][1] += m_sedYield[i];
        m_cellSed[i][0] = m_cellSed[i][1] * m_brt[i];  //this time step
        m_cellSed[i][1] -= m_cellSed[i][0];
        m_cellSed[i][1] = Max(m_cellSed[i][1],0.f);
    }
