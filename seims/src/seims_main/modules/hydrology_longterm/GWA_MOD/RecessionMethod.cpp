...
ReservoirMethod::~ReservoirMethod() {
    ...
    if (gw_height != nullptr) Release1DArray(gw_height);
    if (m_gw_q != nullptr) Release1DArray(m_gw_q);
    if (m_gw_shallow != nullptr) Release1DArray(m_gw_shallow);
}
...
void ReservoirMethod::InitialOutputs() {
    ...
    //ljj++
    if (gw_delaye == nullptr) Initialize1DArray(nLen, gw_delaye, exp(-1./m_delay));
...
# endif
		...
		m_petSubbsn[0] += fPET * curBasinArea;
		m_T_Perco[0] += perco * curBasinArea;
		m_T_PerDep[0] += percoDeep * curBasinArea;
		m_T_Revap[0] += revap * curBasinArea;
		m_T_RG[0] += groundRunoff * curBasinArea; //get rg of specific subbasin
		m_T_QG[0] += groundQ; //get qg of specific subbasin
		m_gwSto[0] += groundStorage * curBasinArea;
    }
...
#pragma omp parallel for
        for (int i = 0; i < nCells; i++) {
            ...
            if (m_soilWtrSto[cells[i]][j] - m_soilSat[cells[i]][j] > 1.e-4f) {
                ...
                m_soilWtrSto[cells[i]][j] = m_soilSat[cells[i]][j];
                for (int ly = CVT_INT(m_nSoilLyrs[cells[i]]) - 2; ly >= 0; ly--) {
                    m_soilWtrSto[cells[i]][ly] += ul_excess;
                    if (m_soilWtrSto[cells[i]][ly] > m_soilSat[cells[i]][ly]) {
                        ul_excess = m_soilWtrSto[cells[i]][ly] - m_soilSat[cells[i]][ly];
                        m_soilWtrSto[cells[i]][ly] = m_soilSat[cells[i]][ly];
                    } else {
                        ul_excess = 0.f;
                        break;
                    }
...
bool ReservoirMethod::CheckInputData() {
    ...
    CHECK_POINTER(MID_GWA_RE, m_area);
    return true;
}
...
void ReservoirMethod::SetValue(const char* key, const float value) {
    ...
    else if (StringMatch(sk, VAR_ALPHABF)) m_alpha_bf = value;
    else if (StringMatch(sk, VAR_DELAY)) m_delay = value;
    else {
        throw ModelException(MID_GWA_RE, "SetValue", "Parameter " + sk + " does not exist in current module.");
    }
}
void ReservoirMethod::SetReaches(clsReaches* reaches) {
    if (nullptr == reaches) {
        throw ModelException(MID_GWA_RE, "SetReaches", "The reaches input can not to be NULL.");
    }
