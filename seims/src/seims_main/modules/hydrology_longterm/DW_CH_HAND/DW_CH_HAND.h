/*! 
 * \file DW_CH_HAND.h
 * \brief Diffusive-wave channel routing with HAND-based inundation states.
 * \author lp
 * \date 2026-03-20
 */
#ifndef SEIMS_MODULE_DW_CH_HAND_H
#define SEIMS_MODULE_DW_CH_HAND_H
...
class DW_CH_HAND : public SimulationModule {
public:
    DW_CH_HAND();
    virtual ~DW_CH_HAND();
...
private:
    ...
    bool HasAnyLakeOrReservoir() const;
    float GetLakeBForControlReach(int lakeId) const;
    float GetMinVolFracForControlReach(int lakeId) const;
    float ComputeLakeOutletCrestStage(int lakeId, float minFrac) const;
    float ComputeLakeExchangeCapacity(int controlLakeId, float remoteStage) const;
    float ApplyLakeExchangeCapacityLimit(int controlLakeId, float qDW, float remoteStage) const;
...
private:
    ...
    // ===== Previous-step hydraulic state =====
    float* m_rivOut_pre;
    float* m_rivDph_pre;
    float* m_subbasinInundationArea_pre;
    float* m_sfcElv_pre;
    float* m_dwnElv_pre;
...
#endif /* SEIMS_MODULE_DW_CH_HAND_H */
