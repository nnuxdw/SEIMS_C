...
const float DW_WET_DEPTH = 0.01f;
const float DW_DRYSTART_MIN_DEPTH = 0.01f;
const float DW_DRYSTART_PREV_DEPTH_FRAC = 0.3f;

// ======================== Outlet boundary condition ========================
// These are hard-coded switches for testing before adding database parameters.
// 0: closed internal-basin outlet, no outflow.
// 1: normal-slope free outflow boundary.
// 2: fixed external water-surface stage boundary.
// 3: zero-depth ghost boundary at outlet bed elevation.
const int DW_OUTLET_BC_CLOSED = 0;
const int DW_OUTLET_BC_NORMAL_SLOPE = 1;
const int DW_OUTLET_BC_FIXED_STAGE = 2;
const int DW_OUTLET_BC_ZERO_DEPTH = 3;

// ---- Manually change this value for tests ----
// Current default keeps your present internal-basin behavior.
//const int DW_OUTLET_BC_TYPE = DW_OUTLET_BC_CLOSED;
const int DW_OUTLET_BC_TYPE = DW_OUTLET_BC_NORMAL_SLOPE;

// For DW_OUTLET_BC_FIXED_STAGE, unit: m.
// Example: external lake/sea/downstream river water level.
const float DW_OUTLET_BC_STAGE = 0.f;

// For DW_OUTLET_BC_NORMAL_SLOPE.
// Example: 1.e-5 with a 10000 m ghost length gives 0.1 m head drop.
const float DW_OUTLET_BC_SLOPE = 1.e-5f;

// Ghost downstream distance, unit: m.
// If <= 0, the module uses m_chLen[i].
const float DW_OUTLET_BC_LENGTH = -1.f;

// Only used for DW_OUTLET_BC_FIXED_STAGE.
// false: external water can block outflow but cannot flow back into the model.
// true : external water can backflow into the model when external stage is higher.
const bool DW_OUTLET_BC_ALLOW_BACKFLOW = false;

// Tiny stage-difference dead zone to avoid numerical oscillation near equal stages.
const float DW_OUTLET_STAGE_DIFF_EPS = 1.e-4f;
...
DW_CH_HAND::DW_CH_HAND() :
    ...
    m_rivOut_pre(nullptr), m_rivDph_pre(nullptr), m_subbasinInundationArea_pre(nullptr),
    m_sfcElv_pre(nullptr), m_dwnElv_pre(nullptr), m_sfcElv(nullptr), m_dwnElv(nullptr),
    ...
    m_minvol(NODATA_VALUE), m_lakeb(NODATA_VALUE), m_lakealpha(nullptr),
    m_minvol_1d(nullptr), m_lakeb_1d(nullptr),
...
DW_CH_HAND::~DW_CH_HAND() {
    if (m_rivOut_pre) Release1DArray(m_rivOut_pre);
    if (m_rivDph_pre) Release1DArray(m_rivDph_pre);
    if (m_subbasinInundationArea_pre) Release1DArray(m_subbasinInundationArea_pre);
    if (m_sfcElv_pre) Release1DArray(m_sfcElv_pre);
    if (m_dwnElv_pre) Release1DArray(m_dwnElv_pre);
...
void DW_CH_HAND::SetValue(const char* key, float value) {
    string sk(key);
    ...
    else throw ModelException(MID_DW_CH_HAND, "SetValue", "Parameter " + sk + " does not exist.");
}

// Override one reach output when the model runs a selected subbasin.
void DW_CH_HAND::SetValueByIndex(const char* key, int index, float value) {
    ...
    else throw ModelException(MID_DW_CH_HAND, "SetValueByIndex", "Parameter " + sk + " does not exist.");
}

// Attach 1-D input arrays from upstream modules, reach data, climate, and HAND rasters.
void DW_CH_HAND::Set1DData(const char* key, int n, float* data) {
    string sk(key);
    if (StringMatch(sk, VAR_SUBBSN)) m_subbsnID = data;
    else if (StringMatch(sk, VAR_SBPET)) { CheckInputSize(MID_DW_CH_HAND, key, n - 1, m_nreach); m_petSubbsn = data; }
    else if (StringMatch(sk, VAR_SBGS)) { CheckInputSize(MID_DW_CH_HAND, key, n - 1, m_nreach); m_gwSto = data; }
    else if (StringMatch(sk, VAR_SBOF)) { CheckInputSize(MID_DW_CH_HAND, key, n - 1, m_nreach); m_olQ2Rch = data; }
    else if (StringMatch(sk, VAR_SBIF)) { CheckInputSize(MID_DW_CH_HAND, key, n - 1, m_nreach); m_ifluQ2Rch = data; }
    else if (StringMatch(sk, VAR_SBQG)) { CheckInputSize(MID_DW_CH_HAND, key, n - 1, m_nreach); m_gndQ2Rch = data; }
    else if (StringMatch(sk, VAR_PCP)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_netPcp = data; }
    else if (StringMatch(sk, VAR_PET)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_PET = data; }
    else if (StringMatch(sk, VAR_OL_HAND_INFIL)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_Infil = data; }
    else if (StringMatch(sk, VAR_HAND_EVAP)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_handEvap = data; }
    else if (StringMatch(sk, VAR_HAND_DEP)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_handDep = data; }
    else if (StringMatch(sk, VAR_OL_HAND_BACK_FROM_GW)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_BackFromGW = data; }
    else if (StringMatch(sk, VAR_AHRU)) {
        CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells);
        m_area = data;
        m_handArea = data;
    }
    ...
    else if (StringMatch(sk, VAR_HAND_Subbasin)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_Subbasin = data; }
    else if (StringMatch(sk, VAR_HAND_Flood_Level)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_Flood_Level = data; }
    else if (StringMatch(sk, VAR_HAND_LevelDepth)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_LevelDepth = data; }
    else if (StringMatch(sk, VAR_HAND_SumArea)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_SumArea = data; }
    else if (StringMatch(sk, VAR_HAND_SumVolume)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_SumVolume = data; }
    else if (StringMatch(sk, VAR_HAND_AvgDepth)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_AvgDepth = data; }
    else if (StringMatch(sk, VAR_HAND_AccVolume)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_AccVolume = data; }
    else if (StringMatch(sk, VAR_HAND_LowerAccDepthFlat)) m_HAND_LowerAccDepthFlat = data;
    else if (StringMatch(sk, VAR_HAND_LowerAccDepthLen)) { CheckInputSize(MID_DW_CH_HAND, key, n, m_nCells); m_HAND_LowerAccDepthLen = data; }
    else if (StringMatch(sk, VAR_DEM) || StringMatch(sk, VAR_RUNOFF_CO) || StringMatch(sk, VAR_SLOPE)) return;
    else throw ModelException(MID_DW_CH_HAND, "Set1DData", "Parameter " + sk + " does not exist.");
}

// Keep the 2-D input hook for framework compatibility.
void DW_CH_HAND::Set2DData(const char* key, int n, int col, float** data) {
    string sk(key);
    if (StringMatch(sk, VAR_SOILT)) return;
    throw ModelException(MID_DW_CH_HAND, "Set2DData", "Parameter " + sk + " does not exist.");
}

// Load reach geometry, routing topology, lake/reservoir flags, and control data.
void DW_CH_HAND::SetReaches(clsReaches* rches) {
    if (nullptr == rches) throw ModelException(MID_DW_CH_HAND, "SetReaches", "The reaches input can not to be NULL.");
    ...
    if (nullptr == m_lakealpha) rches->GetReachesSingleProperty(REACH_LAKEALPHA, &m_lakealpha);
...
void DW_CH_HAND::SetSubbasins(clsSubbasins* subbsns) {
...
void DW_CH_HAND::SetScenario(Scenario* sce) {
    if (nullptr == sce) {
        throw ModelException(MID_DW_CH_HAND, "SetScenario", "The scenario can not to be NULL.");
    }
...
bool DW_CH_HAND::CheckInputData() {
    CHECK_POSITIVE(MID_DW_CH_HAND, m_dt);
    CHECK_NONNEGATIVE(MID_DW_CH_HAND, m_inputSubbsnID);
    CHECK_POSITIVE(MID_DW_CH_HAND, m_nreach);
    CHECK_POSITIVE(MID_DW_CH_HAND, m_outletID);
    CHECK_NODATA(MID_DW_CH_HAND, m_Epch);
    CHECK_NODATA(MID_DW_CH_HAND, m_Bnk0);
    CHECK_NODATA(MID_DW_CH_HAND, m_Chs0_perc);
    CHECK_NODATA(MID_DW_CH_HAND, m_aBank);
    CHECK_NODATA(MID_DW_CH_HAND, m_bBank);
    CHECK_POINTER(MID_DW_CH_HAND, m_subbsnID);
    CHECK_POINTER(MID_DW_CH_HAND, m_petSubbsn);
    CHECK_POINTER(MID_DW_CH_HAND, m_gwSto);
    CHECK_POINTER(MID_DW_CH_HAND, m_olQ2Rch);
    CHECK_POINTER(MID_DW_CH_HAND, m_ifluQ2Rch);
    CHECK_POINTER(MID_DW_CH_HAND, m_gndQ2Rch);
    CHECK_POINTER(MID_DW_CH_HAND, m_chBedElev);
    CHECK_POINTER(MID_DW_CH_HAND, m_ChDepth);
    CHECK_POINTER(MID_DW_CH_HAND, m_outletHandId);
    CHECK_POINTER(MID_DW_CH_HAND, m_islake);
    CHECK_POINTER(MID_DW_CH_HAND, m_isres);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_Subbasin);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_Flood_Level);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_LevelDepth);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_SumArea);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_SumVolume);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_AvgDepth);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_AccVolume);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_LowerAccDepthFlat);
    CHECK_POINTER(MID_DW_CH_HAND, m_HAND_LowerAccDepthLen);
    ...
    if (invalidBedCount > 0) {
        throw ModelException(MID_DW_CH_HAND, "CheckInputData", "CH_BED_ELEV data is missing or invalid.");
    }
    if (invalidHandCount > 0) {
        throw ModelException(MID_DW_CH_HAND, "CheckInputData",
            "CH_OUTLET_HANDID data is invalid for " + ValueToString(invalidHandCount)
            + " reaches." + handMsg.str());
    }

    if (HasAnyLakeOrReservoir()) {
        CHECK_POINTER(MID_DW_CH_HAND, m_lakealpha);
        for (int i = 1; i <= m_nreach; ++i) {
            if (!IsLakeLikeReach(i)) continue;
            const float alpha = m_lakealpha[i];
            if (!IsFiniteF(alpha) || alpha <= 0.f) {
                throw ModelException(MID_DW_CH_HAND, "CheckInputData",
                    "Invalid LAKE_ALPHA for lake/res reach " + ValueToString(i)
                    + ": " + ValueToString(alpha) + ". It must be finite and positive.");
            }
            GetLakeBForControlReach(i);
            GetMinVolFracForControlReach(i);
        }
    }
    return true;
}

// Allocate outputs and initialize channel, lake, and HAND states.
void DW_CH_HAND::InitialOutputs() {
    CHECK_POSITIVE(MID_DW_CH_HAND, m_nreach);
    CHECK_POSITIVE(MID_DW_CH_HAND, m_nCells);
    ...
    m_rivOut_pre = new(nothrow) float[sz]();
    m_rivDph_pre = new(nothrow) float[sz]();
    m_subbasinInundationArea_pre = new(nothrow) float[sz]();
    m_sfcElv_pre = new(nothrow) float[sz]();
    m_dwnElv_pre = new(nothrow) float[sz]();
    ...
    for (int i = 1; i <= m_nreach; ++i) {
        ...
        // Align DW initial channel/lake/reservoir storage with MUSK_CH_HAND.
        // Do NOT initialize ordinary rivers from m_Chs0_perc * channel depth.
        ...
        m_rivDph_pre[i] = m_chWtrDepth[i];
        m_rivOut_pre[i] = 0.f;
        m_sfcElv_pre[i] = m_sfcElv[i];
        m_dwnElv_pre[i] = m_sfcElv[i];
        m_subbasinInundationArea_pre[i] = m_subbasinInundationArea[i];

        // This only initializes lake/reservoir reference depth.
        // It does not affect ordinary river initial storage.
...
void DW_CH_HAND::RefreshReachHydraulicState(int i) {
    ...
    // Build the control-section hydraulic state used by the local inertial equation.
...
int DW_CH_HAND::Execute() {
    ...
    if (nullptr != m_subbasinsInfo && nullptr != m_area) {
        for (auto id = m_subbasinIDs.begin(); id != m_subbasinIDs.end(); ++id) {
            ...
            const double channelStoCap = (!isLakeLike && !isConnector)
                ? static_cast<double>(ComputeRiverChannelStorageCap(reachId)) : 0.0;

            if (handNetSubtractM3 > 0.0) {
                float remainingSubtract = static_cast<float>(handNetSubtractM3);
                if (!isLakeLike && !isConnector) {
                    const float overbankSto = std::max(m_chSto[reachId] - static_cast<float>(channelStoCap), 0.f);
                ...
                if (remainingSubtract > 0.f) {
                    ...
                    remainingSubtract -= removeInBank;
                }
    ...
    for (int step = 0; step < subSteps; ++step) {
        ...
        for (auto it = m_rteLyrs.begin(); it != m_rteLyrs.end(); ++it) {
            ...
            if (errCount > 0) {
                throw ModelException(MID_DW_CH_HAND, "Execute",
...
TimeStepType DW_CH_HAND::GetTimeStepType() {
    return TIMESTEP_CHANNEL;
}

// Return a scalar output for the selected reach.
void DW_CH_HAND::GetValue(const char* key, float* value) {
    InitialOutputs();
    string sk(key);
    ...
    else throw ModelException(MID_DW_CH_HAND, "GetValue", "Parameter " + sk + " does not exist.");
}

// Return reach-indexed 1-D outputs.
void DW_CH_HAND::Get1DData(const char* key, int* n, float** data) {
    InitialOutputs();
    string sk(key);
    *n = m_nreach + 1;
    ...
    else throw ModelException(MID_DW_CH_HAND, "Get1DData", "Output " + sk + " does not exist.");
}

// Return 2-D outputs, currently lake/reservoir water balance only.
void DW_CH_HAND::Get2DData(const char* key, int* n, int* col, float*** data) {
    InitialOutputs();
    string sk(key);
    if (StringMatch(sk, "lake_wb")) {
        ...
        *col = 7;
        ...
        throw ModelException(MID_DW_CH_HAND, "Get2DData", "Parameter " + sk + " does not exist.");
    }
}

// Convert point-source management records to reach inflow rates.
void DW_CH_HAND::PointSourceLoading() {
...
// Route one reach through one diffusive-wave/local-inertial substep.
bool DW_CH_HAND::ChannelFlow_DiffusiveWave(int i, float sub_dt) {
    ...
    // Keep reverse-flow checkpoints for water balance.
    ...
    // Use local and downstream stages to form the water-surface slope.
    const int jseq = static_cast<int>(m_reachDownStream[i]);
    const bool hasDownstream = (jseq > 0 && jseq <= m_nreach);
    const bool isOutletReach = (!hasDownstream && i == m_outletID);
    const bool useOutletBoundary =
        isOutletReach && DW_OUTLET_BC_TYPE != DW_OUTLET_BC_CLOSED;

    const float outletBcLength =
        (DW_OUTLET_BC_LENGTH > 0.f)
            ? DW_OUTLET_BC_LENGTH
            : std::max(m_chLen[i], 1.f);

    float downstreamStage = m_sfcElv[i];
    float slopeLength = std::max(m_chLen[i], 1.f);

    if (hasDownstream) {
        downstreamStage = m_sfcElv[jseq];
        slopeLength = std::max(m_chLen[i], 1.f);
    }
    else if (useOutletBoundary) {
        slopeLength = outletBcLength;

        if (DW_OUTLET_BC_TYPE == DW_OUTLET_BC_NORMAL_SLOPE) {
            const float bcSlope = std::max(DW_OUTLET_BC_SLOPE, 0.f);
            downstreamStage = m_sfcElv[i] - bcSlope * slopeLength;
        }
        else if (DW_OUTLET_BC_TYPE == DW_OUTLET_BC_FIXED_STAGE) {
            downstreamStage = DW_OUTLET_BC_STAGE;
        }
        else if (DW_OUTLET_BC_TYPE == DW_OUTLET_BC_ZERO_DEPTH) {
            downstreamStage = m_chBedElev[i];
        }
        else {
            downstreamStage = m_sfcElv[i];
        }
    }
    else {
        downstreamStage = m_sfcElv[i];
    ...
    float stageUp = m_sfcElv[i];

    if (isConnector && connectorUpLake > 0 && hasDownstream) {
        // Connector flow is controlled by the two adjacent lake/reservoir stages.
        stageUp = m_sfcElv[connectorUpLake];
        downstreamStage = m_sfcElv[jseq];
        m_dwnElv[i] = downstreamStage;
        slopeLength = std::max(m_chLen[i], 1.f);
    }

    float rawSlope = (stageUp - downstreamStage) / std::max(slopeLength, 1.f);

    if (std::fabs(stageUp - downstreamStage) < DW_OUTLET_STAGE_DIFF_EPS) {
        rawSlope = 0.f;
    }
    const float dslp = rawSlope;

    // Build the effective hydraulic state used by the local inertial equation.
    const float hydraulicDepth = std::max(m_chWtrDepth[i], 0.f);
    const float hydraulicArea = std::max(m_chCrossArea[i], 1.e-10f);
    const float effWidth = std::max(m_chWtrWth[i], 1.f);
    const float dPrev = std::max(m_rivDph_pre[i], 0.f);
    float dPrevEff = dPrev;
    if (dPrev <= DW_DRY_DEPTH) {
        // Smooth dry-to-wet starts to avoid a near-zero inertia depth.
        if (hydraulicDepth >= DW_WET_DEPTH) {
            dPrevEff = std::max(DW_DRYSTART_PREV_DEPTH_FRAC * hydraulicDepth, DW_DRYSTART_MIN_DEPTH);
        }
        else {
            dPrevEff = std::max(dPrev, DW_DRYSTART_MIN_DEPTH);
        }
    }
    const float hIm = std::max(0.5f * (hydraulicDepth + dPrevEff), DW_DRYSTART_MIN_DEPTH);
    const float qPrev = m_rivOut_pre[i] / effWidth;
    const float nMain = isLakeLike ? std::max(m_chMan[i], DW_LAKE_MIN_MANNING) : std::max(m_chMan[i], 1.e-4f);
    const float n2 = nMain * nMain;
    const float hImPowNegSevenThirds = powf(hIm, -7.f / 3.f);
    const float liNumerator = qPrev + g * dt * hIm * dslp;
    const float liFrictionTerm = g * dt * n2 * fabsf(qPrev) * hImPowNegSevenThirds;
    const float liDenominator = 1.f + liFrictionTerm;
    // Semi-implicit local inertial update for unit-width discharge.
    float qUnit = liNumerator / liDenominator;

    float dout = effWidth * qUnit;
    // Limit velocity to keep the explicit storage update bounded.
    const float maxVel = isLakeLike ? 2.f : 10.f;
    const float maxDout = hydraulicArea * maxVel;
    dout = std::max(-maxDout, std::min(maxDout, dout));
    if ((!hasDownstream && !useOutletBoundary) || hydraulicArea <= 1.e-8f) {
        dout = 0.f;
    }
    m_rivOut[i] = dout;

    // Apply MUSK-parameter exchange capacity after DW computes the signed
    // hydraulic flux, but before reverse-flow water availability is checked.
    if ((isLakeLike || isConnector) && fabsf(m_rivOut[i]) > UTIL_ZERO) {
        const int controlLakeId = isConnector ? connectorUpLake : i;
        const float remoteStageForExchange =
            isConnector && hasDownstream ? m_sfcElv[jseq] : downstreamStage;
        m_rivOut[i] = ApplyLakeExchangeCapacityLimit(
            controlLakeId, m_rivOut[i], remoteStageForExchange);
    }
    ...
    if (m_rivOut[i] < 0.f) {
        ...
        if (hasDownstream) {
            // Internal reverse flow can only use water actually stored in the downstream reach.
            actBackVol = std::min(reqBackVol, std::max(m_chSto[jseq], 0.f));
            m_chSto[jseq] -= actBackVol;

            if (reqBackVol > UTIL_ZERO && actBackVol > 0.f) {
                m_rivOut[i] *= actBackVol / reqBackVol;
            }
            else {
                m_rivOut[i] = 0.f;
            }
            ...
            if (isConnector && connectorUpLake > 0 && actBackVol > UTIL_ZERO) {
                ...
                RefreshReachHydraulicState(connectorUpLake);
                m_rivOut[i] = 0.f;
            }
        }
        else if (useOutletBoundary) {
            // External reverse flow from ghost boundary.
            // Only fixed-stage boundary is allowed to add external water back.
            if (DW_OUTLET_BC_TYPE == DW_OUTLET_BC_FIXED_STAGE && DW_OUTLET_BC_ALLOW_BACKFLOW) {
                actBackVol = reqBackVol;
            }
            else {
                actBackVol = 0.f;
                m_rivOut[i] = 0.f;
    ...
    m_rivVel[i] = (hydraulicArea > 1.e-8f) ? (m_rivOut[i] / hydraulicArea) : 0.f;
    m_chSto[i] -= m_rivOut[i] * dt;
    if (m_chSto[i] < 0.f) m_chSto[i] = 0.f;
    float routedOut = std::max(m_rivOut[i], 0.f) * dt;
    const float connectorForwardVolCap =
        isConnector ? routedOut : (std::numeric_limits<float>::max)();
    float rttlc = 0.f;
    ...
    if (isConnector) {
        ...
        if (m_chSto[i] > connectorCap + UTIL_ZERO) {
            ...
            float returnedExcess = excessSto;
            if (dischargeToDownstream) {
                // Send excess forward only up to the limited connector exchange volume.
                const float forwardRoom = std::max(connectorForwardVolCap - routedOut, 0.f);
                const float forwardExcess = std::min(excessSto, forwardRoom);
                routedOut += forwardExcess;
                returnedExcess = excessSto - forwardExcess;
            }
            if (connectorUpLake > 0 && returnedExcess > UTIL_ZERO) {
                // Otherwise return excess water to the upstream lake.
                m_chSto[connectorUpLake] += returnedExcess;
                RefreshReachHydraulicState(connectorUpLake);
            }
    ...
    m_seepage[i] = isLakeLike ? lakeSeep : rttlc;
    m_charge[i] = bankOut;
    m_recharge[i] = bankOutGw;

    // Refresh again after routing, seepage and evaporation so the saved state is consistent.
    RefreshReachHydraulicState(i);
    // Save previous-step states for the next local inertial update.
    m_rivOut_pre[i] = m_rivOut[i];
    m_rivDph_pre[i] = m_chWtrDepth[i];
    m_sfcElv_pre[i] = m_sfcElv[i];
    m_dwnElv_pre[i] = m_dwnElv[i];
    m_subbasinInundationArea_pre[i] = m_subbasinInundationArea[i];
...
bool DW_CH_HAND::IsLakeLikeReach(int reachId) const {
    return reachId > 0 && reachId <= m_nreach
        && (m_islake[reachId] == 1.f || m_isres[reachId] == 1.f);
}

// Return true when any reach is flagged as a lake or reservoir.
bool DW_CH_HAND::HasAnyLakeOrReservoir() const {
    if (nullptr == m_islake || nullptr == m_isres || m_nreach <= 0) return false;
    for (int i = 1; i <= m_nreach; ++i) {
        if (m_islake[i] == 1.f || m_isres[i] == 1.f) return true;
    }
    return false;
}

float DW_CH_HAND::GetLakeBForControlReach(int lakeId) const {
    if (lakeId <= 0 || lakeId > m_nreach) {
        throw ModelException(MID_DW_CH_HAND, "GetLakeBForControlReach",
            "Invalid lake/res reach id " + ValueToString(lakeId) + ".");
    }

    if (nullptr != m_lakeb_1d && IsFiniteF(m_lakeb_1d[lakeId]) && m_lakeb_1d[lakeId] > 0.f) {
        return m_lakeb_1d[lakeId];
    }
    if (IsFiniteF(m_lakeb) && m_lakeb > 0.f) {
        return m_lakeb;
    }

    const float reachLakeB = (nullptr != m_lakeb_1d) ? m_lakeb_1d[lakeId] : NODATA_VALUE;
    throw ModelException(MID_DW_CH_HAND, "GetLakeBForControlReach",
        "Invalid lakeb for lake/res reach " + ValueToString(lakeId)
        + ": LAKEB_1D=" + ValueToString(reachLakeB)
        + ", scalar LAKEB=" + ValueToString(m_lakeb)
        + ". A finite positive value is required.");
}

float DW_CH_HAND::GetMinVolFracForControlReach(int lakeId) const {
    if (lakeId <= 0 || lakeId > m_nreach) {
        throw ModelException(MID_DW_CH_HAND, "GetMinVolFracForControlReach",
            "Invalid lake/res reach id " + ValueToString(lakeId) + ".");
    }

    if (nullptr != m_minvol_1d && IsFiniteF(m_minvol_1d[lakeId]) && m_minvol_1d[lakeId] >= 0.f) {
        return m_minvol_1d[lakeId];
    }
    if (IsFiniteF(m_minvol) && m_minvol >= 0.f) {
        return m_minvol;
    }

    const float reachMinVol = (nullptr != m_minvol_1d) ? m_minvol_1d[lakeId] : NODATA_VALUE;
    throw ModelException(MID_DW_CH_HAND, "GetMinVolFracForControlReach",
        "Invalid minvol for lake/res reach " + ValueToString(lakeId)
        + ": LAKE_MNVOL_1D=" + ValueToString(reachMinVol)
        + ", scalar LAKE_MNVOL=" + ValueToString(m_minvol)
        + ". A finite non-negative value is required.");
}

float DW_CH_HAND::ComputeLakeOutletCrestStage(int lakeId, float minFrac) const {
    if (lakeId <= 0 || lakeId > m_nreach) {
        throw ModelException(MID_DW_CH_HAND, "ComputeLakeOutletCrestStage",
            "Invalid lake/res reach id " + ValueToString(lakeId) + ".");
    }
    if (nullptr == m_lakedpini || nullptr == m_lakedp
        || nullptr == m_chBedElev || nullptr == m_ChDepth) {
        throw ModelException(MID_DW_CH_HAND, "ComputeLakeOutletCrestStage",
            "Lake outlet crest calculation is missing depth or elevation arrays.");
    }

    const float refDepth =
        (IsFiniteF(m_lakedpini[lakeId]) && m_lakedpini[lakeId] > 0.f)
            ? m_lakedpini[lakeId]
            : (IsFiniteF(m_lakedp[lakeId]) ? std::max(m_lakedp[lakeId], 0.f) : 0.f);

    const float ctrlGroundElev =
        m_chBedElev[lakeId] + std::max(m_ChDepth[lakeId], 0.f);
    const float lakeBedElevApprox = ctrlGroundElev - refDepth;
    const float thresholdDepth = std::max(minFrac, 0.f) * refDepth;

    return lakeBedElevApprox + thresholdDepth;
}

float DW_CH_HAND::ComputeLakeExchangeCapacity(int controlLakeId, float remoteStage) const {
    if (controlLakeId <= 0 || controlLakeId > m_nreach || !IsLakeLikeReach(controlLakeId)) {
        throw ModelException(MID_DW_CH_HAND, "ComputeLakeExchangeCapacity",
            "Invalid lake/res control reach id " + ValueToString(controlLakeId) + ".");
    }
    if (nullptr == m_lakealpha || nullptr == m_lakearea || nullptr == m_sfcElv) {
        throw ModelException(MID_DW_CH_HAND, "ComputeLakeExchangeCapacity",
            "Lake exchange capacity calculation is missing lakealpha, lakearea, or stage arrays.");
    }
    if (!IsFiniteF(remoteStage)) {
        throw ModelException(MID_DW_CH_HAND, "ComputeLakeExchangeCapacity",
            "Invalid remote stage for control reach " + ValueToString(controlLakeId)
            + ": " + ValueToString(remoteStage) + ".");
    }

    const float alpha = m_lakealpha[controlLakeId];
    const float lakeB = GetLakeBForControlReach(controlLakeId);
    const float minFrac = GetMinVolFracForControlReach(controlLakeId);
    if (!IsFiniteF(alpha) || alpha <= 0.f) {
        throw ModelException(MID_DW_CH_HAND, "ComputeLakeExchangeCapacity",
            "Invalid LAKE_ALPHA for control reach " + ValueToString(controlLakeId)
            + ": " + ValueToString(alpha) + ".");
    }

    const float lakeAreaM2 = std::max(m_lakearea[controlLakeId], 0.f);
    if (lakeAreaM2 <= 0.f) return 0.f;

    const float crestStage = ComputeLakeOutletCrestStage(controlLakeId, minFrac);
    const float localStage =
        std::max(m_sfcElv[controlLakeId], m_chBedElev[controlLakeId]);
    const float hLocal = std::max(localStage - crestStage, 0.f);
    const float hRemote = std::max(remoteStage - crestStage, 0.f);
    const float hCtrl = std::max(hLocal, hRemote);

    // Use m_dt, not sub_dt, so MUSK lakealpha/lakeb keep their calibration meaning
    // under DW sub-stepping.
    const float ratingDt = std::max(static_cast<float>(m_dt), 1.e-6f);

    float qCap = 0.f;
    if (hCtrl > 0.f) {
        qCap = alpha * powf(hCtrl, lakeB) * lakeAreaM2 / ratingDt;
    }
    if (!IsFiniteF(qCap) || qCap < 0.f) {
        throw ModelException(MID_DW_CH_HAND, "ComputeLakeExchangeCapacity",
            "Invalid qCap for control reach " + ValueToString(controlLakeId)
            + ": alpha=" + ValueToString(alpha)
            + ", lakeb=" + ValueToString(lakeB)
            + ", minvol=" + ValueToString(minFrac)
            + ", lakeAreaM2=" + ValueToString(lakeAreaM2)
            + ", remoteStage=" + ValueToString(remoteStage)
            + ", qCap=" + ValueToString(qCap) + ".");
    }
    return qCap;
}

float DW_CH_HAND::ApplyLakeExchangeCapacityLimit(
    int controlLakeId, float qDW, float remoteStage) const {
    if (!IsFiniteF(qDW)) {
        throw ModelException(MID_DW_CH_HAND, "ApplyLakeExchangeCapacityLimit",
            "Invalid DW exchange flux for control reach " + ValueToString(controlLakeId)
            + ": " + ValueToString(qDW) + ".");
    }
    if (fabsf(qDW) <= UTIL_ZERO) return 0.f;

    const float qCap = ComputeLakeExchangeCapacity(controlLakeId, remoteStage);
    const float qAbs = std::min(fabsf(qDW), qCap);
    return qDW > 0.f ? qAbs : -qAbs;
}

// Find the single upstream lake/reservoir feeding a connector reach.
int DW_CH_HAND::GetLakeConnectorUpstreamLake(int reachId) const {
...
bool DW_CH_HAND::IsLakeConnectorReach(int reachId) const {
...
float DW_CH_HAND::ComputeLakeConnectorStage(int reachId) const {
...
float DW_CH_HAND::ComputeRiverChannelStorageCap(int reachId) const {
    if (reachId <= 0 || reachId > m_nreach) return 0.f;
...
float DW_CH_HAND::ComputeRiverChannelOutletDepth(int reachId, float sto) const {
...
void DW_CH_HAND::SetRiverChannelOnlyOutletHandDepth(int reachId, float depth) {
...
bool DW_CH_HAND::HandInundation_BinarySearch(int reachId, float sto) {
...
void DW_CH_HAND::updateAllHandsWtrDep(int reachId) {
...
void DW_CH_HAND::ClearHandStateForReach(int reachId) {
    if (reachId <= 0) return;
...
void DW_CH_HAND::LoadHandLevelsFromArrays(int cellsNum, int flatLen,
    ...
    if (!m_HAND_Subbasin || !m_HAND_Flood_Level || !m_HAND_LevelDepth
        ...
        throw ModelException(MID_DW_CH_HAND, "LoadHandLevelsFromArrays",
            "HAND arrays are not loaded.");
    }
    if (cellsNum <= 0) {
        throw ModelException(MID_DW_CH_HAND, "LoadHandLevelsFromArrays",
            "HAND cell count must be positive.");
    }
    ...
    if (maxSbid < 0) {
        throw ModelException(MID_DW_CH_HAND, "LoadHandLevelsFromArrays",
            "No valid HAND records were found in the input arrays.");
    }
    ...
    for (int i = 0; i < cellsNum; ++i) {
        ...
        if (L > 0) {
            if (flatPos + L > flatLen) {
                throw ModelException(MID_DW_CH_HAND, "LoadHandLevelsFromArrays",
                    "HAND LowerAccDepthFlat length is shorter than expected.");
            }
            ...
            if (nullptr == level.m_levelLowerAccDepth) {
                throw ModelException(MID_DW_CH_HAND, "LoadHandLevelsFromArrays",
                    "Allocation failed for HAND lower accumulated depth.");
            }
    ...
    if (buildHandIds) {
        for (auto& kv : idsTmp) {
            ...
            if (level.m_levelHandNum > 0) {
                ...
                if (nullptr == level.handIds) {
                    throw ModelException(MID_DW_CH_HAND, "LoadHandLevelsFromArrays",
                        "Allocation failed for HAND cell ids.");
                }
...
float DW_CH_HAND::ComputeResScheduledOutflow(int i, float curSto, float qIn, float sub_dt) {
