...
def field_param_csv(csv_file,field_shp_file, raster_para_files, lakesubareaidTXT, soilgrids0idTXT):
    ...
    # # ok.（因为soiltype就是subarea，并通过subarea分区统计，所以众数和均值是一样的.但是HYDRO_GROUP是根据clay等值的范围给的值，clay是均值得到的）
    ...
    #         'SOL_CRK','ANION_EXCL','SOL_SUMUL','DET_CLAY','SOL_SUMAWC','SOL_ALB','DET_SMAGG','DET_LGAGG']#均值
    #
    ...
    # ok
    # land_init = ['CURYR_INIT','BIO_INIT','RSDIN','EPCO','LAI_INIT','CHT','DORMI','PHU_PLT']#均值
    # ok
    # land_init_type = ['LANDCOVER','IGRO']#众数

    # other_param = ['CN2','dayLenMin','dem','depression','dormhr','USLE_P','slope','slope_dinf','moist_in','runoff_co','acc','dist2Stream']#均值

    other_param = ['dayLenMin', 'dem', 'depression', 'runoff_co', 'dormhr',  'slope', 'slope_dinf', 'moist_in',
                    'acc', 'dist2Stream']  # 均值
    # runoff_co 【USLE_P移到land_init中】
    ...
    # param_lists_majority = [, soil_typedata, crop, land_init_type]
    # param_lists_majority_file = ['landuse_lookup', 'soil_typedata', 'crop', 'land_init_type']

    param_lists_mean = [other_param]
    param_lists_mean_file = ['other_param']
...
def get_coord_field_point(field_center_file_84, field_center, csv_file):
    ...
    fcsv.write('FID,celllong,celllat,prjX,prjY\n')
...
if __name__ == "__main__":
    ...
    # # 转换为 pyproj.CRS 对象（以便 geopandas 使用）
    # target_crs = CRS.from_wkt(target_srs.ExportToWkt())
    # # 投影并计算面积
    # gdf_projected = gdf.to_crs(target_crs)
    # gdf['CELLAREA'] = gdf_projected.area
    # result_df = gdf[['FIELDID', 'CELLAREA']].copy()
    # result_df.rename(columns={'FIELDID': 'FID'}, inplace=True)  # 重命名为 FIELFID
    # result_df[['FID', 'CELLAREA']].to_csv(csv_file, index=False)
    # print('cumpute HRU area done!')

    driver = ogr.GetDriverByName("ESRI Shapefile")
    ds = driver.Open(HRU_shp, 0)  # 0 = read-only
    if ds is None:
        raise FileNotFoundError(f"无法打开文件: {HRU_shp}")
    layer = ds.GetLayer()
    original_crs = layer.GetSpatialRef()
    if original_crs is None:
        original_crs = osr.SpatialReference()
        original_crs.ImportFromEPSG(4326)  # 默认 WGS84
        layer.SetSpatialRef(original_crs)
    target_srs = osr.SpatialReference()
    mollweide_wkt = """
        PROJCS["World_Mollweide",
            GEOGCS["GCS_WGS_1984",
        ...
        """
    target_srs.ImportFromWkt(mollweide_wkt)
    coord_transform = osr.CoordinateTransformation(original_crs, target_srs)
    results = []
    temp_dict = {}  # 临时字典，键是 FID，值是累加的 CELLAREA
    for feature in layer:
        geom = feature.GetGeometryRef()
        if geom is None:
            continue
        projected_geom = geom.Clone()
        projected_geom.Transform(coord_transform)
        area = projected_geom.GetArea()
        field_id = feature.GetField("FIELDID")
        # results.append({"FID": field_id, "CELLAREA": area})
        # 累加面积到字典
        if field_id in temp_dict:
            temp_dict[field_id] += area
        else:
            temp_dict[field_id] = area

    results = [{"FID": fid, "CELLAREA": area} for fid, area in temp_dict.items()]

    ds = None
    with open(csv_file, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["FID", "CELLAREA"])
        writer.writeheader()
